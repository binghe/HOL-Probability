...
open HolKernel Parse boolLib bossLib metisLib
     combinTheory pred_setTheory extra_pred_setTheory 
     extra_realTheory arithmeticTheory realTheory realLib pairTheory 
      seqTheory transcTheory;
...
val REAL_SUM_IMAGE_POW = store_thm
 ("REAL_SUM_IMAGE_POW",``!a s. FINITE s 
           ==> ((SIGMA a s) pow 2 = SIGMA (\(i,j). a i * a j) (s CROSS s):real)``,
  RW_TAC std_ss []
...
val REAL_SUM_IMAGE_EQ = store_thm
 ("REAL_SUM_IMAGE_EQ", ``!s (f:'a -> real) f'. FINITE s /\ (!x. x IN s ==> (f x = f' x)) ==> (SIGMA f s = SIGMA f' s)``,
  RW_TAC std_ss []
...
val REAL_SUM_IMAGE_SUB = store_thm
 ("REAL_SUM_IMAGE_SUB", ``!s (f:'a -> real) f'. FINITE s ==> 
  ...
  ++ RW_TAC std_ss [Once real_sub,REAL_SUM_IMAGE_ADD,Once REAL_NEG_MINUS1,REAL_SUM_IMAGE_CMUL]
...
val entire = store_thm
  ...
  REPEAT Cases ++ RW_TAC std_ss [extreal_mul_def,num_not_infty,extreal_of_num_def,extreal_11,REAL_ENTIRE]);
...
val le_02 = store_thm
("le_02",``0 <= 2``, RW_TAC real_ss [extreal_of_num_def,extreal_le_def]);

val lt_02 = store_thm
("lt_02",``0 < 2``, RW_TAC real_ss [extreal_of_num_def,extreal_lt_def,extreal_le_def]);

val ne_02 = store_thm
("ne_02",``0 <> 2``, RW_TAC real_ss [extreal_of_num_def]);
...
val le_lneg = store_thm
  ...
  REPEAT Cases ++ RW_TAC std_ss [extreal_ainv_def,extreal_le_def,extreal_add_def,extreal_sub_def,le_infty,extreal_of_num_def,extreal_not_infty,REAL_LE_LNEG]);
...
("let_mul", ``!x y. 0 <= x /\ 0 < y ==> 0 <= x * y``,
  REPEAT Cases
  ++ RW_TAC real_ss [extreal_mul_def,extreal_le_def,extreal_lt_eq,lt_infty,le_infty,le_refl,extreal_of_num_def]
...
("mul_let",``!x y. 0 <= x /\ y < 0 ==> x * y <= 0``,
 Cases ++ Cases
  ++ RW_TAC real_ss [extreal_lt_eq,extreal_mul_def,extreal_of_num_def,le_refl,le_infty,lt_infty,extreal_le_def]
  ++ METIS_TAC [REAL_LT_NEG,REAL_LT_IMP_LE,REAL_NEG_0,REAL_LE_MUL,REAL_MUL_RNEG,REAL_NEG_NEG,REAL_LE_NEG,real_lt,REAL_LT_LE]);
...
("abs_bounds",``!x k. abs x <= k <=> -k <= x /\ x <= k``,
  Cases ++ Cases ++ RW_TAC std_ss [extreal_abs_def,extreal_le_def,lt_infty,le_infty,extreal_ainv_def,ABS_BOUNDS]);

val abs_bounds_lt = store_thm
("abs_bounds_lt",``!x k. abs x < k <=> -k < x /\ x < k``,
  Cases ++ Cases ++ RW_TAC std_ss [extreal_abs_def,extreal_lt_eq,lt_infty,le_infty,extreal_ainv_def]
  ++ REAL_ARITH_TAC);
...
("add_infty",``(!x. (x + PosInf = PosInf) /\ (PosInf + x = PosInf)) /\
               (!x. x <> PosInf ==> ((x + NegInf = NegInf) /\ (NegInf + x = NegInf)))``,
  RW_TAC std_ss [] ++ Cases_on `x` ++ RW_TAC std_ss [extreal_add_def,extreal_not_infty]);
...
val lt_sub = store_thm
  ("lt_sub",``!x y z. z <> NegInf /\ z <> PosInf ==> (y + x < z = y < z - x)``,
 REPEAT Cases
...
("neg_eq0", ``!x. (-x = 0) <=> (x = 0)``,
 Cases ++ RW_TAC std_ss [extreal_ainv_def,extreal_of_num_def,REAL_NEG_EQ0]);

val eq_neg = store_thm
("eq_neg", ``!x y. (-x = -y) <=> (x = y)``,
 REPEAT Cases ++ RW_TAC std_ss [extreal_ainv_def,REAL_EQ_NEG]);
...
val add2_sub2 = store_thm
  ("add2_sub2", ``!a b c d. (b <> PosInf /\ d <> PosInf) \/ (b <> NegInf /\ d <> NegInf) ==> (a - b + (c - d) = (a + c - (b + d)))``,
  REPEAT Cases
  ++ RW_TAC std_ss [extreal_add_def,extreal_sub_def]
  ++ REAL_ARITH_TAC);

val sub_ldistrib = store_thm
  ("sub_ldistrib", ``!x y z. x <> NegInf /\ x <> PosInf /\ y <> NegInf /\ y <> PosInf /\ z <> NegInf /\ z <> PosInf ==> (x * (y - z) = x * y - x * z)``,
  REPEAT Cases ++ RW_TAC real_ss [extreal_add_def,extreal_sub_def,extreal_mul_def,REAL_SUB_LDISTRIB]);

val sub_rdistrib = store_thm
  ("sub_rdistrib", ``!x y z. x <> NegInf /\ x <> PosInf /\ y <> NegInf /\ y <> PosInf /\ z <> NegInf /\ z <> PosInf ==> ((x - y) * z = x * z - y * z)``,
  REPEAT Cases ++ RW_TAC real_ss [extreal_add_def,extreal_sub_def,extreal_mul_def,REAL_SUB_RDISTRIB]);
...
("inv_1over",``!x. inv x = 1 / x``,
  REPEAT Cases
  ++ RW_TAC real_ss [extreal_inv_def,extreal_div_def,extreal_mul_def,extreal_of_num_def,REAL_10,REAL_INV1]);
...
val inv_pos = store_thm
  ...
  Cases
  ++ RW_TAC real_ss [extreal_div_def,extreal_inv_def,extreal_of_num_def,extreal_lt_def,extreal_mul_def,extreal_le_def,lt_infty,le_infty]
  ++ FULL_SIMP_TAC real_ss [real_lte]);

val rinv_uniq = store_thm
 ("rinv_uniq",``!x y. (x * y = 1) ==> (y = inv x)``,
  REPEAT Cases 
  ++ RW_TAC real_ss [extreal_inv_def,extreal_mul_def,extreal_of_num_def,REAL_RINV_UNIQ]);
...
val lt_rdiv_neg = store_thm
 ("lt_rdiv_neg", ``!x y z. z < 0 ==> (y / Normal z < x <=> x * Normal z < y)``,
  ...
  ++ RW_TAC std_ss [REAL_INV_POS,REAL_LE_LT,GSYM real_lte,REAL_INV_EQ_0,REAL_INV_POS,extreal_lt_eq,REAL_LT_RDIV_EQ_NEG,GSYM real_div,REAL_LT_REFL,lt_refl,lt_infty,le_infty,extreal_div_def,extreal_inv_def,extreal_div_eq,extreal_mul_def,REAL_LT_REFL])
  ++ METIS_TAC [REAL_LT_REFL,REAL_LT_ANTISYM,real_lte,REAL_LT_IMP_LE,REAL_LT_INV_EQ]);

val div_add = store_thm
  ("div_add",``!x y z.  x <> NegInf /\ y <> NegInf /\ z <> 0 ==> (x / z + y / z = (x + y) / z)``,
  REPEAT Cases
  ++ RW_TAC real_ss [extreal_add_def,extreal_div_def,extreal_mul_def,extreal_inv_def,extreal_of_num_def]
  ++ RW_TAC real_ss [extreal_add_def,REAL_RDISTRIB]);

val le_inv = store_thm
 ("le_inv",``!x. 0 <= x ==> 0 <= inv x``,
  Cases ++ RW_TAC real_ss [extreal_inv_def,extreal_of_num_def,extreal_le_def,le_infty,REAL_LE_INV]);
...
val logr_not_infty = store_thm
 ("logr_not_infty",``!x b. (x <> NegInf /\ x <> PosInf) ==> logr b x <> NegInf /\ logr b x <> PosInf``,
...
val half_cancel = store_thm
 ("half_cancel",``2 * (1 / 2) = 1``,
   RW_TAC real_ss [extreal_of_num_def,extreal_mul_def,extreal_div_eq,EVAL ``2 <> 0:real``,REAL_MUL_RINV,real_div]);

val third_cancel = store_thm
 ("third_cancel",``3 * (1 / 3) = 1``,
   RW_TAC real_ss [extreal_of_num_def,extreal_mul_def,extreal_div_eq,EVAL ``3 <> 0:real``,REAL_MUL_RINV,real_div]);

val fourth_cancel = store_thm
 ("fourth_cancel",``4 * (1 / 4) = 1``,
   RW_TAC real_ss [extreal_of_num_def,extreal_mul_def,extreal_div_eq,EVAL ``4 <> 0:real``,REAL_MUL_RINV,real_div]);
...
val REAL_LE_MUL_EPSILON = store_thm
  ("REAL_LE_MUL_EPSILON",
   ``!x y:real. (!z. 0 < z /\ z < 1 ==> z * x <= y) ==>
    ...
    REPEAT STRIP_TAC
    ++ Cases_on `x = 0`
    >> (Q.PAT_ASSUM `!z. P z` (MP_TAC o Q.SPEC `1/2`) ++ RW_TAC real_ss [REAL_HALF_BETWEEN])
    ++ Cases_on `0 < x`
...
("EXTREAL_SUM_IMAGE_NOT_NEG_INF",``!f s. FINITE s /\ (!x. x IN s ==> f x <> NegInf) ==>
  ...
  Suff `!s. FINITE s ==> (\s. !f. (!x. x IN s ==> f x <> NegInf) ==> EXTREAL_SUM_IMAGE f s <> NegInf) s`
  >> RW_TAC std_ss []
...
("EXTREAL_SUM_IMAGE_NOT_POS_INF",``!f s. FINITE s /\ (!x. x IN s ==> f x <> PosInf) ==>
  ...
  Suff `!s. FINITE s ==> (\s. !f. (!x. x IN s ==> f x <> PosInf) ==> EXTREAL_SUM_IMAGE f s <> PosInf) s`
  >> RW_TAC std_ss []
  ++ MATCH_MP_TAC FINITE_INDUCT
  ++ RW_TAC std_ss [EXTREAL_SUM_IMAGE_THM,num_not_infty]
  ++ METIS_TAC [add_not_infty,IN_INSERT,DELETE_NON_ELEMENT]);
...
val EXTREAL_SUM_IMAGE_FINITE_SAME = store_thm
  ("EXTREAL_SUM_IMAGE_FINITE_SAME",
   ``!s. FINITE s ==>
	 !f p. p IN s /\ (!q. q IN s ==> (f p = f q)) ==> (EXTREAL_SUM_IMAGE f s = (&(CARD s)) * f p)``,
  Suff `!s. FINITE s ==>
	 (\s. !f p. p IN s /\ (!q. q IN s ==> (f p = f q)) ==> (EXTREAL_SUM_IMAGE f s = (&(CARD s)) * f p)) s`
  >> METIS_TAC []
  ++ MATCH_MP_TAC FINITE_INDUCT
...
val EXTREAL_SUM_IMAGE_IN_IF = store_thm
  ("EXTREAL_SUM_IMAGE_IN_IF",
   ``!s. FINITE s ==>
	!f. EXTREAL_SUM_IMAGE f s = EXTREAL_SUM_IMAGE (\x. if x IN s then f x else 0) s``,
  Suff `!s. FINITE s ==>
		(\s. !f. EXTREAL_SUM_IMAGE f s = EXTREAL_SUM_IMAGE (\x. if x IN s then f x else 0) s) s`
  >> METIS_TAC []
  ++ MATCH_MP_TAC FINITE_INDUCT
...
val EXTREAL_SUM_IMAGE_CMUL = store_thm
  ("EXTREAL_SUM_IMAGE_CMUL",
   ``!s. FINITE s ==>
	!f c. (0 <= c) \/ (!x. x IN s ==> 0 <= f x) ==> (EXTREAL_SUM_IMAGE (\x. Normal (c) * f x) s = Normal (c) * (EXTREAL_SUM_IMAGE f s))``,
  Suff `!s. FINITE s ==>
	(\s. !f c. (0 <= c) \/ (!x. x IN s ==> 0 <= f x) ==> (EXTREAL_SUM_IMAGE (\x. Normal (c) * f x) s = Normal (c) * (EXTREAL_SUM_IMAGE f s))) s`
  >> METIS_TAC []
  ++ MATCH_MP_TAC FINITE_INDUCT
...
val EXTREAL_SUM_IMAGE_CMUL2 = store_thm
  ...
  Suff `!s. FINITE s ==>
	(\s. !f c. (!x. x IN s ==> f x <> NegInf) ==> (EXTREAL_SUM_IMAGE (\x. Normal (c) * f x) s = Normal (c) * (EXTREAL_SUM_IMAGE f s))) s`
  >> METIS_TAC []
  ++ MATCH_MP_TAC FINITE_INDUCT
  ++ RW_TAC real_ss [EXTREAL_SUM_IMAGE_THM,mul_rzero,DELETE_NON_ELEMENT]
  ++ Cases_on `c = 0` >> RW_TAC std_ss [GSYM extreal_of_num_def,mul_lzero,add_lzero,EXTREAL_SUM_IMAGE_0]
  ++ Cases_on `0 < c`
...
val EXTREAL_SUM_IMAGE_IMAGE = store_thm
  ("EXTREAL_SUM_IMAGE_IMAGE",
   ``!s. FINITE s ==>
	!f'. INJ f' s (IMAGE f' s) ==> (!f. EXTREAL_SUM_IMAGE f (IMAGE f' s) = EXTREAL_SUM_IMAGE (f o f') s)``,
  Suff `!s. FINITE s ==>
	(\s.!f'. INJ f' s (IMAGE f' s) ==> (!f. EXTREAL_SUM_IMAGE f (IMAGE f' s) = EXTREAL_SUM_IMAGE (f o f') s)) s`
  >> METIS_TAC []
  ++ MATCH_MP_TAC FINITE_INDUCT
...
val EXTREAL_SUM_IMAGE_DISJOINT_UNION = store_thm
  ("EXTREAL_SUM_IMAGE_DISJOINT_UNION",
   ``!s s'. FINITE s /\ FINITE s' /\ DISJOINT s s' ==>
		(!f. EXTREAL_SUM_IMAGE f (s UNION s') = EXTREAL_SUM_IMAGE f s + EXTREAL_SUM_IMAGE f s')``,
  Suff `!s. FINITE s ==> (\s. !s'. FINITE s' ==>
		(\s'. DISJOINT s s' ==>
			(!f. EXTREAL_SUM_IMAGE f (s UNION s') = EXTREAL_SUM_IMAGE f s + EXTREAL_SUM_IMAGE f s')) s') s`
  >> METIS_TAC []
  ++ MATCH_MP_TAC FINITE_INDUCT
...
val EXTREAL_SUM_IMAGE_INTER_NONZERO = store_thm
  ("EXTREAL_SUM_IMAGE_INTER_NONZERO",
   ``!s. FINITE s ==>
	!f. EXTREAL_SUM_IMAGE f (s INTER (\p. ~(f p = 0))) = EXTREAL_SUM_IMAGE f s``,
  Suff `!s. FINITE s ==> (\s. !f. EXTREAL_SUM_IMAGE f (s INTER (\p. ~(f p = 0))) = EXTREAL_SUM_IMAGE f s) s`
  >> METIS_TAC []
  ++ MATCH_MP_TAC FINITE_INDUCT
...
val EXTREAL_SUM_IMAGE_ZERO_DIFF = store_thm
 ("EXTREAL_SUM_IMAGE_ZERO_DIFF",``!s. FINITE s ==> !f t. (!x. x IN t ==> (f x = 0)) 
  ...
  RW_TAC std_ss []
  ++ `s = (s DIFF t) UNION (s INTER t)` by (RW_TAC std_ss [EXTENSION,IN_INTER,IN_UNION,IN_DIFF] ++ METIS_TAC [])
  ++ `FINITE (s DIFF t) /\ FINITE (s INTER t)` by METIS_TAC [INTER_FINITE,FINITE_DIFF]
  ++ `DISJOINT (s DIFF t) (s INTER t)` by (RW_TAC std_ss [DISJOINT_DEF,IN_INTER,NOT_IN_EMPTY,EXTENSION,IN_DIFF] ++ METIS_TAC [])
...
val EXTREAL_SUM_IMAGE_MONO_SET = store_thm
  ...
  ++ `DISJOINT s (t DIFF s)` by (`DISJOINT s (t DIFF s)` by RW_TAC std_ss [DISJOINT_DEF,IN_DIFF,EXTENSION,GSPECIFICATION,NOT_IN_EMPTY,IN_INTER] ++ METIS_TAC [])
...
val EXTREAL_SUM_IMAGE_ADD = store_thm
  ("EXTREAL_SUM_IMAGE_ADD",
   ``!s. FINITE s ==> !f f'. 
	(EXTREAL_SUM_IMAGE (\x. f x + f' x) s = EXTREAL_SUM_IMAGE f s + EXTREAL_SUM_IMAGE f' s)``,
  Suff `!s. FINITE s ==>
	(\s. !f f'. (EXTREAL_SUM_IMAGE (\x. f x + f' x) s = EXTREAL_SUM_IMAGE f s + EXTREAL_SUM_IMAGE f' s)) s`
  >> RW_TAC std_ss []
  ++ MATCH_MP_TAC FINITE_INDUCT
...
val EXTREAL_SUM_IMAGE_SUB = store_thm
  ("EXTREAL_SUM_IMAGE_SUB",
   ``!s. FINITE s ==> !f f'. (!x. x IN s ==> f' x <> NegInf) \/ (!x. x IN s ==> f' x <> PosInf) ==> (EXTREAL_SUM_IMAGE (\x. f x - f' x) s =
		 EXTREAL_SUM_IMAGE f s - EXTREAL_SUM_IMAGE f' s)``,
  Suff `!s. FINITE s ==>
...
val EXTREAL_SUM_IMAGE_NORMAL = store_thm
  ...
  >> RW_TAC std_ss []
  ++ MATCH_MP_TAC FINITE_INDUCT
  ++ RW_TAC std_ss [EXTREAL_SUM_IMAGE_THM,extreal_of_num_def,REAL_SUM_IMAGE_THM,GSYM extreal_add_def,DELETE_NON_ELEMENT]);

val EXTREAL_SUM_IMAGE_IN_IF_ALT = store_thm
("EXTREAL_SUM_IMAGE_IN_IF_ALT",``!s f z. FINITE s ==> (EXTREAL_SUM_IMAGE f s = EXTREAL_SUM_IMAGE (\x. if x IN s then f x else z) s)``,
  Suff `!s. FINITE s ==> (\s. !f z. EXTREAL_SUM_IMAGE f s = EXTREAL_SUM_IMAGE (\x. if x IN s then f x else z) s) s`
  >> METIS_TAC []
  ++ MATCH_MP_TAC FINITE_INDUCT
  ++ RW_TAC std_ss [EXTREAL_SUM_IMAGE_THM,DELETE_NON_ELEMENT]
...
("EXTREAL_SUM_IMAGE_COUNT",``!f. (EXTREAL_SUM_IMAGE f (count 2) = f 0 + f 1) /\ 
  ...
  ++ `{1:num; 2; 3; 4} DELETE 0 = {1;2;3;4}` by RW_TAC real_ss [EXTENSION,IN_DELETE,IN_SING,IN_INSERT]
  ++ FULL_SIMP_TAC real_ss [FINITE_SING,FINITE_INSERT,EXTREAL_SUM_IMAGE_THM,EXTREAL_SUM_IMAGE_SING,add_assoc,IN_INSERT,NOT_IN_EMPTY]
  ++ DECIDE_TAC);
  ...
  ++ `sup (IMAGE (\n. f e n + (\n. SIGMA (\i. f i n) s) n) UNIV) =
      sup (IMAGE (f e) UNIV) + sup (IMAGE (\n. SIGMA (\i. f i n) s) UNIV)`
        by ((MATCH_MP_TAC o Q.SPECL [`f e`, `(\n. SIGMA (\i. f i n) s)`] o INST_TYPE [alpha |-> ``:num``]) sup_add_mono 
            ++ FULL_SIMP_TAC std_ss [IN_INSERT,EXTREAL_SUM_IMAGE_POS,EXTREAL_SUM_IMAGE_MONO])
  ++ METIS_TAC [IN_INSERT]);
...
("ext_suminf_add",``!f g. (!n. 0 <= f n /\ 0 <= g n) ==> (ext_suminf (\n. f n + g n) = ext_suminf f + ext_suminf g)``,
...
("ext_suminf_sum",``!f n. (!n. 0 <= f n) /\ (!m. n <= m ==> (f m = 0)) ==> (ext_suminf f = SIGMA f (count n))``,
  RW_TAC std_ss [ext_suminf_def,sup_eq]
  >> (POP_ASSUM (MP_TAC o ONCE_REWRITE_RULE [GSYM SPECIFICATION])
      ++ RW_TAC std_ss [IN_IMAGE,IN_UNIV]
      ++ Cases_on `n' <= n`
