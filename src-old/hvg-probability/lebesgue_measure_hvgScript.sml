...
lebesgue_hvgTheory probability_hvgTheory;

val _ = new_theory "lebesgue_measure_hvg";

val IN_FUN_SET = store_thm ("IN_FUN_SET",
...
val borel_open = store_thm ("borel_open",
  ``!A. topology_hvg$Open A ==> A IN subsets borel``,
  SIMP_TAC std_ss [borel, sigma_def, subsets_def] THEN
...
val open_UNION_box = store_thm ("open_UNION_box",
  ``!M. topology_hvg$Open M ==> (M = BIGUNION {box a b | box a b SUBSET M})``,
...
val open_union_box = store_thm ("open_union_box",
  ``!M. topology_hvg$Open M ==> (M = BIGUNION
...
(* ported to examples/probability/lebesgue_measureScript.sml
...
(* ported as measure_space_density_of
...
val Radon_Nikodym_finite_measure_infinite = store_thm ("Radon_Nikodym_finite_measure_infinite",
 ``!M N. measure_space M /\ measure_space N /\
         finite_measure M /\ measure_absolutely_continuous N M /\
         (measurable_sets M = measurable_sets N) ==>
         ?f. f IN measurable (m_space M,measurable_sets M) Borel /\
