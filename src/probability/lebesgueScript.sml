...
val integrable_normal = store_thm (* new *)
  ("integrable_normal",
...
val finite_space_integral_reduce = store_thm
  ...
  ``!m f. measure_space m /\ f IN measurable (m_space m,measurable_sets m) Borel /\
         (!x. x IN m_space m ==> f x <> NegInf /\ f x <> PosInf) /\
          FINITE (m_space m) /\ integrable m f /\ measure m (m_space m) < PosInf
     ==> (integral m f = finite_space_integral m f)``,
    rpt STRIP_TAC
...
val finite_POW_RN_deriv_reduce = store_thm
 ...
 >> RW_TAC std_ss [] (* 2 subgoals *)
 >- (Q.EXISTS_TAC (* i.e. `v {x} / measure m {x} := 0` if `measure m {x} = 0` *)
         `\x. if measure m {x} <> 0 then v {x} / measure m {x} else 0`
     >> SIMP_TAC std_ss []
     >> STRONG_CONJ_TAC
 ...
 *)
val MARKOV_INEQUALITY_MSPACE = store_thm
  ("MARKOV_INEQUALITY_MSPACE",
