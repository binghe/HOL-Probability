...
val REAL_LT_BETWEEN = store_thm ("REAL_LT_BETWEEN",
 ``!a b:real. a < b <=> ?x. a < x /\ x < b``,
  rpt GEN_TAC THEN EQ_TAC THENL [ALL_TAC, MESON_TAC[REAL_LT_TRANS]] THEN
...
val FINREC_1_LEMMA = store_thm ("FINREC_1_LEMMA",
  ...
  REWRITE_TAC[FINREC] THEN rpt GEN_TAC THEN AP_TERM_TAC THEN ABS_TAC THEN
  ...
  ``!(f:'a->'b->'b) b.
           ...
           ==> !n s z.
                  ...
                  ==> !x. x IN s ==> ?w. FINREC f b (s DELETE x) w n /\
                                         (z = f x w)``,
  rpt GEN_TAC THEN DISCH_TAC THEN INDUCT_TAC THENL
  [REWRITE_TAC[FINREC_1_LEMMA] THEN REWRITE_TAC[FINREC] THEN
  rpt GEN_TAC THEN STRIP_TAC THEN STRIP_TAC THEN
  ASM_REWRITE_TAC[IN_INSERT, NOT_IN_EMPTY] THEN
  DISCH_THEN SUBST1_TAC THEN EXISTS_TAC ``b:'b`` THEN
  ASM_REWRITE_TAC[SING_DELETE], rpt GEN_TAC THEN
...
val FINREC_UNIQUE_LEMMA = store_thm ("FINREC_UNIQUE_LEMMA",
  ...
  REWRITE_TAC[FINREC] THEN MESON_TAC[NOT_IN_EMPTY],
  REWRITE_TAC[FINREC] THEN MESON_TAC[NOT_IN_EMPTY],
  IMP_RES_THEN ASSUME_TAC FINREC_SUC_LEMMA THEN rpt GEN_TAC THEN
...
val FINREC_EXISTS_LEMMA = store_thm ("FINREC_EXISTS_LEMMA",
  ...
  MATCH_MP_TAC FINITE_INDUCT THEN BETA_TAC THEN rpt STRIP_TAC THENL
...
val FINREC_FUN_LEMMA = store_thm ("FINREC_FUN_LEMMA",
  ...
  rpt STRIP_TAC THEN EXISTS_TAC ``\s:'a. @a:'b. ?n:'c. R s a n`` THEN
  rpt STRIP_TAC THEN BETA_TAC THEN EQ_TAC THENL [STRIP_TAC THEN
...
val FINREC_FUN = store_thm ("FINREC_FUN",
 ``!(f:'a->'b->'b) b.
  ...
  rpt STRIP_TAC THEN IMP_RES_THEN MP_TAC FINREC_UNIQUE_LEMMA THEN
  rpt STRIP_TAC THEN KNOW_TAC ``!n1 n2 s a1 a2. FINREC f b s a1 n1 /\
...
val SET_RECURSION_LEMMA = store_thm ("SET_RECURSION_LEMMA",
 ``!(f:'a->'b->'b) b.
  ...
  rpt GEN_TAC THEN COND_CASES_TAC THEN ASM_REWRITE_TAC[] THEN
  DISCH_TAC THENL
   [AP_TERM_TAC THEN REWRITE_TAC[GSYM ABSORPTION] THEN ASM_REWRITE_TAC[],
    SUBGOAL_THEN ``FINITE(x:'a INSERT s) /\ x IN (x INSERT s)`` MP_TAC THENL
     [REWRITE_TAC[IN_INSERT] THEN ASM_MESON_TAC[FINITE_INSERT],
      ...
      rpt AP_TERM_TAC THEN UNDISCH_TAC ``~(x:'a IN s)`` THEN DISCH_TAC THEN
...
val FINITE_RECURSION = store_thm ("FINITE_RECURSION",
 ``!(f:'a->'b->'b) b.
  ...
  rpt GEN_TAC THEN DISCH_TAC THEN REWRITE_TAC[ITSET] THEN
...
val CARD_UNION_EQ = store_thm ("CARD_UNION_EQ",
  ...
  rpt STRIP_TAC THEN KNOW_TAC ``FINITE (s:'a->bool) /\ FINITE (t:'a->bool)``
...
val FINITE_REAL_INTERVAL = store_thm ("FINITE_REAL_INTERVAL",
 ``(!a. ~FINITE {x:real | a < x}) /\
  ...
  ASSUME_TAC THENL
   [rpt GEN_TAC THEN REWRITE_TAC[GSYM REAL_NOT_LT] THEN
    ...
    [rpt STRIP_TAC THEN SIMP_TAC std_ss [REAL_EQ_LADD, real_div, REAL_EQ_LMUL] THEN
    ...
    ASM_REWRITE_TAC[] THEN rpt CONJ_TAC THEN rpt GEN_TAC THENL
...
val CHOOSE_SUBSET_STRONG = store_thm ("CHOOSE_SUBSET_STRONG",
 ``!n s:'a->bool.
  ...
  rpt(CONJ_TAC THENL [ASM_SET_TAC[], ALL_TAC]) THEN
...
val lemma4 = prove (
 ``!P:real->bool.
    ...
    ((?x. P x) /\ (?s. !x. P x ==> x <= s)
       ==> ?s. (!x. P x ==> x <= s) /\
               !M'. (!x. P x ==> x <= M') ==> s <= M')``,
  rpt STRIP_TAC THEN REWRITE_TAC [GSYM lemma3] THEN
...
val SUP = store_thm ("SUP",
 ``!s:real->bool. ~(s = {}) /\ (?b. !x. x IN s ==> x <= b)
  ...
  rpt STRIP_TAC THEN REWRITE_TAC [SPECIFICATION] THEN
...
val REAL_SUP_UNIQUE = store_thm ("REAL_SUP_UNIQUE",
 ``!s b:real. (!x. x IN s ==> x <= b) /\
         (!b'. b' < b ==> ?x. x IN s /\ b' < x)
         ==> (sup s = b)``,
  rpt STRIP_TAC THEN REWRITE_TAC[sup_alt] THEN MATCH_MP_TAC SELECT_UNIQUE THEN
...
val REAL_SUP_LE_SUBSET = store_thm ("REAL_SUP_LE_SUBSET",
 ``!s t:real->bool. ~(s = {}) /\ s SUBSET t /\ (?b. !x. x IN t ==> x <= b)
         ==> sup s <= sup t``,
  rpt STRIP_TAC THEN MATCH_MP_TAC REAL_SUP_LE_S THEN
...
val SUP_UNIQUE = store_thm ("SUP_UNIQUE",
 ``!s b:real. (!c. (!x. x IN s ==> x <= c) <=> b <= c) ==> (sup s = b)``,
  rpt STRIP_TAC THEN GEN_REWR_TAC RAND_CONV [GSYM SUP_SING] THEN
...
val REAL_LE_INF_SUBSET = store_thm ("REAL_LE_INF_SUBSET",
 ``!s t:real->bool. ~(t = {}) /\ t SUBSET s /\ (?b. !x. x IN s ==> b <= x)
         ==> inf s <= inf t``,
  rpt STRIP_TAC THEN MATCH_MP_TAC REAL_LE_INF THEN
...
val FINITE_NUMSEG = store_thm ("FINITE_NUMSEG",
  ``!m n. FINITE (m..n)``,
  rpt GEN_TAC THEN REWRITE_TAC [numseg] THEN
...
val DISJOINT_NUMSEG = store_thm ("DISJOINT_NUMSEG",
 ``!m n p q. DISJOINT (m..n) (p..q) <=> n < p \/ q < m \/ n < m \/ q < p``,
  ...
  rpt GEN_TAC THEN REWRITE_TAC[DE_MORGAN_THM, NOT_LESS_EQUAL] THEN
...
val NUMSEG_OFFSET_IMAGE = store_thm ("NUMSEG_OFFSET_IMAGE",
 ``!m n p. (m+p..n+p) = IMAGE (\i. i + p) (m..n)``,
  ...
  rpt GEN_TAC THEN BETA_TAC THEN EQ_TAC THENL
...
val SUBSET_NUMSEG = store_thm ("SUBSET_NUMSEG",
 ``!m n p q. (m..n) SUBSET (p..q) <=> n < m \/ p <= m /\ n <= q``,
  rpt GEN_TAC THEN REWRITE_TAC[SUBSET_DEF, IN_NUMSEG] THEN
...
val SUPPORT_CLAUSES = store_thm ("SUPPORT_CLAUSES",
 ``(!f. support op f {} = {}) /\
  ...
  rpt STRIP_TAC THEN TRY COND_CASES_TAC THEN ASM_MESON_TAC[]);
...
val SUPPORT_DELTA = store_thm ("SUPPORT_DELTA",
 ``!op s f a. support op (\x. if x = a then f(x) else neutral op) s =
  ...
  rpt GEN_TAC THEN rpt COND_CASES_TAC THEN
...
val FINITE_SUPPORT_DELTA = store_thm ("FINITE_SUPPORT_DELTA",
 ``!op f a. FINITE(support op (\x. if x = a then f(x) else neutral op) s)``,
  REWRITE_TAC[SUPPORT_DELTA] THEN rpt GEN_TAC THEN COND_CASES_TAC THEN
...
val ITERATE_CLAUSES_GEN = store_thm ("ITERATE_CLAUSES_GEN",
 ``!op. monoidal op
  ...
  [ASM_MESON_TAC [monoidal], FULL_SIMP_TAC std_ss [] THEN rpt STRIP_TAC THEN
...
val lemma = prove (
 ``!t s. t SUBSET s ==> (s = (s DIFF t) UNION t) /\ DISJOINT (s DIFF t) t``,
  rpt STRIP_TAC THENL [SIMP_TAC std_ss [UNION_DEF, DIFF_DEF, EXTENSION, GSPECIFICATION]
...
val lemma1 = prove (
 ``!a b. a UNION b = ((a DIFF b) UNION (b DIFF a)) UNION (a INTER b)``,
  rpt GEN_TAC THEN REWRITE_TAC [UNION_DEF, DIFF_DEF, INTER_DEF]
 ...
 ``!op. monoidal op
        ==> !P. P(neutral op) /\ (!x y. P x /\ P y ==> P (op x y))
                ==> !f:'a->'b s. (!x. x IN s /\ ~(f x = neutral op) ==> P(f x))
                               ==> P(iterate op s f)``,
  rpt STRIP_TAC THEN ONCE_REWRITE_TAC[ITERATE_EXPAND_CASES] THEN
  rpt(POP_ASSUM MP_TAC) THEN REWRITE_TAC[GSYM IN_SUPPORT] THEN
...
val ITERATE_IMAGE = store_thm ("ITERATE_IMAGE",
 ``!op. monoidal op
  ...
  REWRITE_TAC[o_THM, IN_INSERT] THEN rpt STRIP_TAC THEN COND_CASES_TAC THENL
  [METIS_TAC[IN_IMAGE], METIS_TAC[IN_IMAGE]]], GEN_TAC THEN DISCH_TAC
  THEN ONCE_REWRITE_TAC[ITERATE_EXPAND_CASES] THEN rpt STRIP_TAC THEN
  MATCH_MP_TAC lemma THEN REWRITE_TAC[SUPPORT_CLAUSES] THEN rpt STRIP_TAC THENL
...
val ITERATE_BIJECTION = store_thm ("ITERATE_BIJECTION",
 ``!op. monoidal op
  ...
  rpt STRIP_TAC THEN MATCH_MP_TAC EQ_TRANS THEN
...
val ITERATE_ITERATE_PRODUCT = store_thm ("ITERATE_ITERATE_PRODUCT",
 ``!op. monoidal op
  ...
  IN_INSERT] THEN rpt STRIP_TAC THEN
  FIRST_ASSUM(fn th =>
...
val ITERATE_INJECTION = store_thm ("ITERATE_INJECTION",
 ``!op. monoidal op
  ...
  rpt STRIP_TAC THEN CONV_TAC SYM_CONV THEN
...
val ITERATE_UNION_NONZERO = store_thm ("ITERATE_UNION_NONZERO",
 ``!op. monoidal op
        ==> !f:'a->'b s t.
                ...
                ==> (iterate op (s UNION t) f =
                    op (iterate op s f) (iterate op t f))``,
  rpt STRIP_TAC THEN ONCE_REWRITE_TAC[GSYM ITERATE_SUPPORT] THEN
...
val ITERATE_IMAGE_NONZERO = store_thm ("ITERATE_IMAGE_NONZERO",
 ``!op. monoidal op
  ...
  REWRITE_TAC[IN_INSERT] THEN rpt STRIP_TAC THEN
...
val NSUM_DEGENERATE = store_thm ("NSUM_DEGENERATE",
 ``!f s. ~(FINITE {x | x IN s /\ ~(f x = 0:num)}) ==> (nsum s f = 0:num)``,
  rpt GEN_TAC THEN REWRITE_TAC[nsum] THEN
...
val NSUM_LMUL = store_thm ("NSUM_LMUL",
 ``!f c s:'a->bool. nsum s (\x. c * f(x)) = c * nsum s f``,
  rpt GEN_TAC THEN ASM_CASES_TAC ``c = 0:num`` THEN
...
val NSUM_LE = store_thm ("NSUM_LE",
 ``!f g s. FINITE(s) /\ (!x. x IN s ==> f(x) <= g(x))
           ==> nsum s f <= nsum s g``,
  ONCE_REWRITE_TAC[GSYM AND_IMP_INTRO] THEN rpt GEN_TAC THEN
...
val NSUM_LT = store_thm ("NSUM_LT",
 ``!f g s:'a->bool.
        ...
        (?x. x IN s /\ f(x) < g(x))
         ==> nsum s f < nsum s g``,
  rpt GEN_TAC THEN
  rpt(DISCH_THEN(CONJUNCTS_THEN2 ASSUME_TAC MP_TAC)) THEN
...
val NSUM_EQ_0_IFF = store_thm ("NSUM_EQ_0_IFF",
 ``!s. FINITE s ==> ((nsum s f = 0:num) <=> !x. x IN s ==> (f x = 0:num))``,
  rpt STRIP_TAC THEN EQ_TAC THEN ASM_SIMP_TAC std_ss [NSUM_EQ_0] THEN
...
val NSUM_POS_LT_ALL = store_thm ("NSUM_POS_LT_ALL",
 ``!s f:'a->num.
  ...
  rpt STRIP_TAC THEN MATCH_MP_TAC NSUM_POS_LT THEN
...
val NSUM_RESTRICT = store_thm ("NSUM_RESTRICT",
 ``!f s. FINITE s ==> (nsum s (\x. if x IN s then f(x) else 0:num) = nsum s f)``,
  rpt STRIP_TAC THEN MATCH_MP_TAC NSUM_EQ THEN ASM_SIMP_TAC std_ss []);
...
val NSUM_BOUND_GEN = store_thm ("NSUM_BOUND_GEN",
 ``!s f b. FINITE s /\ ~(s = {}) /\ (!x:'a. x IN s ==> f(x) <= b DIV (CARD s))
           ==> nsum s f <= b``,
  rpt STRIP_TAC THEN KNOW_TAC ``0 < CARD s`` THENL
...
val NSUM_BOUND_LT = store_thm ("NSUM_BOUND_LT",
 ``!s f b. FINITE s /\ (!x:'a. x IN s ==> f x <= b) /\ (?x. x IN s /\ f x < b)
           ==> nsum s f < (CARD s) * b``,
  rpt STRIP_TAC THEN MATCH_MP_TAC LESS_LESS_EQ_TRANS THEN
...
val NSUM_RESTRICT_SET = store_thm ("NSUM_RESTRICT_SET",
 ``!P s f. nsum {x:'a | x IN s /\ P x} f = nsum s (\x. if P x then f(x) else 0:num)``,
  ...
  rpt GEN_TAC THEN MATCH_MP_TAC NSUM_EQ THEN SIMP_TAC std_ss [GSPECIFICATION]);
...
val NSUM_NSUM_RESTRICT = store_thm ("NSUM_NSUM_RESTRICT",
 ``!R f s t.
  ...
  rpt GEN_TAC THEN SIMP_TAC std_ss [NSUM_RESTRICT_SET] THEN
...
val NSUM_MULTICOUNT_GEN = store_thm ("NSUM_MULTICOUNT_GEN",
 ``!R:'a->'b->bool s t k.
        ...
        ==> (nsum s (\i. (CARD {j | j IN t /\ R i j})) =
             nsum t (\i. (k i)))``,
  rpt GEN_TAC THEN REWRITE_TAC[CONJ_ASSOC] THEN
  DISCH_THEN(CONJUNCTS_THEN ASSUME_TAC) THEN
  MATCH_MP_TAC EQ_TRANS THEN
  EXISTS_TAC ``nsum s (\i:'a. nsum {j:'b | j IN t /\ R i j} (\j. 1:num))`` THEN
  CONJ_TAC THENL
   [MATCH_MP_TAC NSUM_EQ THEN ASM_REWRITE_TAC[] THEN rpt STRIP_TAC THEN
...
val NSUM_IMAGE_GEN = store_thm ("NSUM_IMAGE_GEN",
 ``!f:'a->'b g s.
  ...
  rpt STRIP_TAC THEN MATCH_MP_TAC EQ_TRANS THEN EXISTS_TAC
...
val NSUM_GROUP = store_thm ("NSUM_GROUP",
 ``!f:'a->'b g s t.
  ...
  rpt STRIP_TAC THEN BETA_TAC THEN MATCH_MP_TAC NSUM_EQ_0 THEN
...
val NSUM_SUBSET_SIMPLE = store_thm ("NSUM_SUBSET_SIMPLE",
 ``!u v f. FINITE v /\ u SUBSET v ==> nsum u f <= nsum v f``,
  rpt STRIP_TAC THEN MATCH_MP_TAC NSUM_SUBSET THEN
...
val NSUM_LE_GEN = store_thm ("NSUM_LE_GEN",
 ``!f g s. (!x:'a. x IN s ==> f x <= g x) /\ FINITE {x | x IN s /\ ~(g x = 0:num)}
           ==> nsum s f <= nsum s g``,
  rpt STRIP_TAC THEN ONCE_REWRITE_TAC[GSYM NSUM_SUPPORT] THEN
...
val NSUM_BIGUNION_NONZERO = store_thm ("NSUM_BIGUNION_NONZERO",
 ``!f s. FINITE s /\ (!t:'a->bool. t IN s ==> FINITE t) /\
  ...
  DISCH_THEN(fn th => STRIP_TAC THEN MP_TAC th) THEN rpt STRIP_TAC THEN
...
val NSUM_CLOSED = store_thm ("NSUM_CLOSED",
 ``!P f:'a->num s.
        ...
        ==> P(nsum s f)``,
  rpt STRIP_TAC THEN MP_TAC(MATCH_MP ITERATE_CLOSED MONOIDAL_ADD) THEN
...
val NSUM_SWAP_NUMSEG = store_thm ("NSUM_SWAP_NUMSEG",
 ``!a b c d f.
  ...
  rpt GEN_TAC THEN MATCH_MP_TAC NSUM_SWAP THEN REWRITE_TAC[FINITE_NUMSEG]);
...
val MOD_NSUM_MOD = store_thm ("MOD_NSUM_MOD",
 ``!f:'a->num n s.
  ...
  rpt STRIP_TAC THEN ASSUME_TAC MOD_PLUS THEN
...
val CARD_BIGUNION = store_thm ("CARD_BIGUNION",
 ``!s:('a->bool)->bool.
  ...
  ASM_SIMP_TAC std_ss [NSUM_CLAUSES] THEN rpt STRIP_TAC THEN
...
val SUM_DEGENERATE = store_thm ("SUM_DEGENERATE",
 ``!f s. ~(FINITE {x | x IN s /\ ~(f x = &0)}) ==> (sum s f = &0)``,
  rpt GEN_TAC THEN REWRITE_TAC[sum_def] THEN
...
val SUM_LMUL = store_thm ("SUM_LMUL",
 ``!f c s:'a->bool. sum s (\x. c * f(x)) = c * sum s f``,
  rpt GEN_TAC THEN ASM_CASES_TAC ``c = 0:real`` THEN
...
val SUM_POS_LT_ALL = store_thm ("SUM_POS_LT_ALL",
 ``!s f:'a->real.
     FINITE s /\ ~(s = {}) /\ (!i. i IN s ==> (0:real) < f i)
       ==> (0:real) < sum s f``,
  rpt STRIP_TAC THEN MATCH_MP_TAC SUM_POS_LT THEN
  ASM_MESON_TAC[MEMBER_NOT_EMPTY, REAL_LT_IMP_LE]);
...
val SUM_ABS_LE = store_thm ("SUM_ABS_LE",
 ``!f:'a->real g s.
        ...
        ==> abs(sum s f) <= sum s g``,
  rpt STRIP_TAC THEN MATCH_MP_TAC REAL_LE_TRANS THEN
...
val SUM_ZERO_EXISTS = store_thm ("SUM_ZERO_EXISTS",
 ``!(u:'a->real) s.
  ...
  rpt STRIP_TAC THEN REPEAT_TCL DISJ_CASES_THEN ASSUME_TAC
...
val SUM_SWAP = store_thm ("SUM_SWAP",
 ``!f:'a->'b->real s t.
  ...
  rpt GEN_TAC THEN KNOW_TAC ``(FINITE (t:'b->bool) ==>
...
val SUM_UNION_RZERO = store_thm ("SUM_UNION_RZERO",
 ``!f:'a->real u v.
  ...
  rpt STRIP_TAC THEN SUBGOAL_THEN ``u UNION v = u UNION (v DIFF u)``
...
val SUM_RESTRICT = store_thm ("SUM_RESTRICT",
 ``!f s. FINITE s ==> (sum s (\x. if x IN s then f(x) else (0:real)) = sum s f)``,
  rpt STRIP_TAC THEN MATCH_MP_TAC SUM_EQ THEN ASM_SIMP_TAC std_ss []);
...
val SUM_BOUND_LT = store_thm ("SUM_BOUND_LT",
 ``!s f b. FINITE s /\ (!x:'a. x IN s ==> f x <= b) /\ (?x. x IN s /\ f x < b)
           ==> sum s f < &(CARD s) * b``,
  rpt STRIP_TAC THEN MATCH_MP_TAC REAL_LTE_TRANS THEN
...
val SUM_UNION_EQ = store_thm ("SUM_UNION_EQ",
 ``!s t u. FINITE u /\ (s INTER t = {}) /\ (s UNION t = u)
  ...
  rpt STRIP_TAC THEN POP_ASSUM MP_TAC THEN GEN_REWR_TAC LAND_CONV [EQ_SYM_EQ]
...
val SUM_RESTRICT_SET = store_thm ("SUM_RESTRICT_SET",
 ``!P s f. sum {x | x IN s /\ P x} f = sum s (\x. if P x then f x else (0:real))``,
  ...
  REWRITE_TAC[METIS [] ``~((if P x then f x else a) = a) <=> P x /\ ~(f x = a)``,
              GSYM CONJ_ASSOC] THEN
  rpt GEN_TAC THEN MATCH_MP_TAC SUM_EQ THEN SIMP_TAC std_ss [GSPECIFICATION]);
...
val SUM_SUM_RESTRICT = store_thm ("SUM_SUM_RESTRICT",
 ``!R f s t.
  ...
  rpt GEN_TAC THEN SIMP_TAC std_ss [SUM_RESTRICT_SET] THEN ASSUME_TAC SUM_SWAP
...
val SUM_MULTICOUNT_GEN = store_thm ("SUM_MULTICOUNT_GEN",
 ``!R:'a->'b->bool s t k.
  ...
  CONJ_TAC THENL
   [MATCH_MP_TAC SUM_EQ THEN ASM_REWRITE_TAC[] THEN rpt STRIP_TAC THEN
...
val SUM_GROUP = store_thm ("SUM_GROUP",
 ``!f:'a->'b g s t.
  ...
  rpt STRIP_TAC THEN BETA_TAC THEN MATCH_MP_TAC SUM_EQ_0 THEN
  FULL_SIMP_TAC std_ss [GSPECIFICATION, IN_IMAGE] THEN METIS_TAC []);
...
val SUM_SUBSET_SIMPLE = store_thm ("SUM_SUBSET_SIMPLE",
 ``!u v f. FINITE v /\ u SUBSET v /\ (!x:'a. x IN (v DIFF u) ==> (0:real) <= f(x))
           ==> sum u f <= sum v f``,
  rpt STRIP_TAC THEN MATCH_MP_TAC SUM_SUBSET THEN
  ASM_MESON_TAC[IN_DIFF, SUBSET_DEF, SUBSET_FINITE]);
...
val SUM_CASES_1 = store_thm ("SUM_CASES_1",
 ``!s a. FINITE s /\ a IN s
  ...
  rpt STRIP_TAC THEN ASM_SIMP_TAC std_ss [SUM_CASES] THEN
...
val SUM_IMAGE_LE = store_thm ("SUM_IMAGE_LE",
 ``!f:'a->'b g s.
        ...
        ==> sum (IMAGE f s) g <= sum s (g o f)``,
  rpt STRIP_TAC THEN MATCH_MP_TAC SUM_LE_INCLUDED THEN
...
val SUM_CLOSED = store_thm ("SUM_CLOSED",
 ``!P f:'a->real s.
        ...
        ==> P(sum s f)``,
  rpt STRIP_TAC THEN MP_TAC(MATCH_MP ITERATE_CLOSED MONOIDAL_REAL_ADD) THEN
...
val SUM_DIFFS_ALT = store_thm ("SUM_DIFFS_ALT",
 ``!m n. sum(m..n) (\k. f(k + 1) - f(k)) =
  ...
  rpt GEN_TAC THEN ONCE_REWRITE_TAC[GSYM REAL_NEG_SUB] THEN
...
val SUM_COMBINE_R = store_thm ("SUM_COMBINE_R",
 ``!f m n p. m <= n + 1 /\ n <= p
  ...
  rpt STRIP_TAC THEN MATCH_MP_TAC SUM_UNION_EQ THEN
...
val SUM_CONG = store_thm
  ...
  rpt STRIP_TAC THEN MATCH_MP_TAC SUM_EQ THEN
  ASM_SIMP_TAC std_ss [GSPECIFICATION,  IN_NUMSEG]);
...
val REAL_SUB_POW = store_thm ("REAL_SUB_POW",
 ``!x y n.
  ...
  SIMP_TAC std_ss [GSYM pow, ADD1, ARITH_PROVE
    ...
    (SUC(n - 1 - x) = n - x)``] THEN rpt STRIP_TAC THEN
  ...
  rpt STRIP_TAC THEN FULL_SIMP_TAC arith_ss [], DISC_RW_KILL THEN
...
val REAL_SUB_POLYFUN = store_thm ("REAL_SUB_POLYFUN",
 ``!a x y n. 1 <= n
  ...
  [MATCH_MP_TAC SUM_EQ THEN rpt STRIP_TAC THEN BETA_TAC THEN
  ...
  rpt(EXISTS_TAC ``\(x:num,y:num). (y,x)``) THEN
...
val REAL_SUB_POLYFUN_ALT = store_thm ("REAL_SUB_POLYFUN_ALT",
 ``!a x y n.
  ...
  rpt STRIP_TAC THEN ASM_SIMP_TAC std_ss [REAL_SUB_POLYFUN] THEN AP_TERM_TAC THEN
  MATCH_MP_TAC SUM_EQ_NUMSEG THEN X_GEN_TAC ``j:num`` THEN rpt STRIP_TAC THEN
  BETA_TAC THEN AP_THM_TAC THEN AP_TERM_TAC THEN
  MATCH_MP_TAC SUM_EQ_GENERAL_INVERSES THEN
  MAP_EVERY EXISTS_TAC
   [``\i:num. i - (j + 1)``, ``\k:num. j + k + 1``] THEN
  REWRITE_TAC[IN_NUMSEG] THEN rpt STRIP_TAC THEN
  RW_TAC arith_ss []);
...
val REAL_POLYFUN_FINITE_ROOTS = store_thm ("REAL_POLYFUN_FINITE_ROOTS",
 ``!n c. FINITE {x | sum(0:num..n) (\i. c i * x pow i) = 0:real} <=>
  ...
  rpt GEN_TAC THEN REWRITE_TAC[TAUT `a /\ ~b <=> ~(a ==> b)`] THEN
...
val REAL_POLYFUN_EQ_0 = store_thm ("REAL_POLYFUN_EQ_0",
 ``!n c. (!x. sum(0:num..n) (\i. c i * x pow i) = 0:real) <=>
         (!i. i IN 0:num..n ==> (c i = 0:real))``,
  rpt GEN_TAC THEN EQ_TAC THEN DISCH_TAC THENL
...
val REAL_POLYFUN_EQ_CONST = store_thm ("REAL_POLYFUN_EQ_CONST",
 ``!n c k. (!x. sum(0:num..n) (\i. c i * x pow i) = k) <=>
  ...
  rpt GEN_TAC THEN MATCH_MP_TAC EQ_TRANS THEN EXISTS_TAC
...
val POLYNOMIAL_FUNCTION_INDUCT = store_thm ("POLYNOMIAL_FUNCTION_INDUCT",
 ``!P. P (\x. x) /\ (!c. P (\x. c)) /\
  ...
  [rpt GEN_TAC THEN ASM_SIMP_TAC arith_ss [SUM_CLAUSES_LEFT, ADD1,
