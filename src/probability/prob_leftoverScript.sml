...
Theorem FUBINI :
    !(X :'a set) (Y :'b set) A B u v f.
     ...
     (* then all three integrals are finite (P /\ Q /\ R) *)
       ...
       (AE y::(Y,B,v). integrable (X,A,u) (\x. f (x,y))) /\        (* (i) *)
       (AE x::(X,A,u). integrable (Y,B,v) (\y. f (x,y))) /\        (* (ii) *)
        integrable (X,A,u) (\x. integral (Y,B,v) (\y. f (x,y))) /\ (* (iii) *)
        integrable (Y,B,v) (\y. integral (X,A,u) (\x. f (x,y))) /\ (* (iv) *)
       (integral ((X,A,u) CROSS (Y,B,v)) f =
        integral (Y,B,v) (\y. integral (X,A,u) (\x. f (x,y)))) /\
       (integral ((X,A,u) CROSS (Y,B,v)) f =
        integral (X,A,u) (\x. integral (Y,B,v) (\y. f (x,y))))     (* (v) *)
Proof
    rpt GEN_TAC
 ...
 (* applying TONELLI again on f^+ and f^- *)
 ...
 >- (REWRITE_TAC [lt_infty] \\
     ...
     MATCH_MP_TAC pos_fn_integral_mono \\
     rw [FN_MINUS_POS, FN_MINUS_LE_ABS]) >> DISCH_TAC
 (* goal (i): AE y::(Y,B,v). integrable (X,A,u) (\x. f (x,y)) *)
 >> STRONG_CONJ_TAC
 >- (rw [Once FN_DECOMP, integrable_def] \\
  (* push ‘fn_plus/fn_minus’ inside *)
    ‘!y. fn_plus (\x. f (x,y)) = (\x. (fn_plus f) (x,y))’
        by rw [FUN_EQ_THM, FN_PLUS_ALT] >> POP_ORW \\
    ‘!y. fn_minus (\x. f (x,y)) = (\x. (fn_minus f) (x,y))’
        by rw [FUN_EQ_THM, FN_MINUS_ALT] >> POP_ORW \\
  (* applying pos_fn_integral_infty_null *)
     ...
     rw [GSPECIFICATION] >> art [] \\
    ‘!x. (fn_plus f) (x,y) - (fn_minus f) (x,y) = f (x,y)’ by METIS_TAC [FN_DECOMP] >> POP_ORW \\
     ONCE_REWRITE_TAC [IN_MEASURABLE_BOREL_PLUS_MINUS] \\
    ‘!y. fn_plus (\x. f (x,y)) = (\x. (fn_plus f) (x,y))’
        by rw [FUN_EQ_THM, FN_PLUS_ALT] >> POP_ORW \\
    ‘!y. fn_minus (\x. f (x,y)) = (\x. (fn_minus f) (x,y))’
        by rw [FUN_EQ_THM, FN_MINUS_ALT] >> POP_ORW \\
     CONJ_TAC >> FIRST_X_ASSUM MATCH_MP_TAC >> art []) >> DISCH_TAC
 (* goal (ii): AE x::(X,A,u). integrable (Y,B,v) (\y. f (x,y)) *)
 >> STRONG_CONJ_TAC
 >- (rw [Once FN_DECOMP, integrable_def] \\
  (* push ‘fn_plus/fn_minus’ inside *)
    ‘!x. fn_plus (\y. f (x,y)) = (\y. (fn_plus f) (x,y))’
        by rw [FUN_EQ_THM, FN_PLUS_ALT] >> POP_ORW \\
    ‘!x. fn_minus (\y. f (x,y)) = (\y. (fn_minus f) (x,y))’
        by rw [FUN_EQ_THM, FN_MINUS_ALT] >> POP_ORW \\
  (* applying pos_fn_integral_infty_null *)
     ...
     CONJ_TAC >- (PROVE_TAC [NULL_SET_UNION, GSYM IN_NULL_SET]) \\
     rw [GSPECIFICATION] >> art [] \\
    ‘!y. (fn_plus f) (x,y) - (fn_minus f) (x,y) = f (x,y)’ by METIS_TAC [FN_DECOMP] >> POP_ORW \\
     ONCE_REWRITE_TAC [IN_MEASURABLE_BOREL_PLUS_MINUS] \\
    ‘!x. fn_plus (\y. f (x,y)) = (\y. (fn_plus f) (x,y))’
        by rw [FUN_EQ_THM, FN_PLUS_ALT] >> POP_ORW \\
    ‘!x. fn_minus (\y. f (x,y)) = (\y. (fn_minus f) (x,y))’
        by rw [FUN_EQ_THM, FN_MINUS_ALT] >> POP_ORW \\
     CONJ_TAC >> FIRST_X_ASSUM MATCH_MP_TAC >> art []) >> DISCH_TAC
 (* goal (iii): integrable (X,A,u) (\x. integral (Y,B,v) (\y. f (x,y))) *)
 >> STRONG_CONJ_TAC
 >- (
     cheat)
(*
     STRIP_ASSUME_TAC
       (SIMP_RULE std_ss [AE_THM, almost_everywhere_def, m_space_def]
                         (ASSUME “AE (y :'b)::((Y,B,v) :'b m_space).
                                     integrable ((X,A,u) :'a m_space) (\x. f (x,y))”)) \\
     MATCH_MP_TAC integrable_eq >> rw [] \\
     Q.EXISTS_TAC ‘\x. integral (Y,B,v) (\y. f (x,y) * indicator_fn (Y DIFF N) y)’ \\
     BETA_TAC \\
     reverse CONJ_TAC
     >- (rpt STRIP_TAC \\
         Know ‘integral (Y,B,v) (\y. f (x,y)) =
               integral (Y,B,v) (\y. f (x,y) * indicator_fn N y) +
               integral (Y,B,v) (\y. f (x,y) * indicator_fn (m_space (Y,B,v) DIFF N) y)’
         >- (HO_MATCH_MP_TAC integral_split' >> art [] \\
             reverse CONJ_TAC >- FULL_SIMP_TAC std_ss [null_set_def] \\
 *)     

(*
     rw [integrable_def] >| (* 3 subgoals *)
     [ (* goal 1 (of 3) *)
      ‘!x y. f (x,y) = fn_plus f (x,y) - fn_minus f (x,y)’
...
(* another form using all ‘integral’s *)
Theorem FUBINI' :
    !(X :'a set) (Y :'b set) A B u v f.
        sigma_finite_measure_space (X,A,u) /\
        sigma_finite_measure_space (Y,B,v) /\
...
Proof
    rpt GEN_TAC
 (* prevent from separating ‘P \/ Q \/ R’ *)
 >> REWRITE_TAC [DECIDE “A /\ B /\ C /\ D ==> E <=> A ==> B ==> C ==> D ==> E”]
 >> rpt DISCH_TAC
