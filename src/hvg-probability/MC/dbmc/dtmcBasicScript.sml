...
val th_dtmc_def = Define `
    ...
    (!t i j. Trans X p s (t + 1) 1 i j = Trans X p s t 1 i j)`;
...
val HOMO_PROPERTY = store_thm
  ("HOMO_PROPERTY",
  ``!(X:num -> 'a -> 'c) p s Linit Ltrans i j n t. 
        th_dtmc X p s Linit Ltrans ==> 
...
(* ------------------------------------------------------------------------- *)	
val PROB_SUBSET_ZERO = store_thm
  ("PROB_SUBSET_ZERO",
...
val SUM_ROW_MATRIX_EQ_ONE = store_thm 
  ...
  ``!X (t:num) j m Linit Ltrans p s.
	th_dtmc X p s Linit Ltrans /\ (0 < distribution p (X t) {j}) ==> 
 ...
 << [FULL_SIMP_TAC std_ss [th_dtmc_def, dtmc_def, mc_property_def, random_variable_def],
     FULL_SIMP_TAC std_ss [th_dtmc_def],
...
(* --------------------------------------------------------------------------*)    		 
val TH_DTMC_NSTEP_COND_PROB = store_thm 
  ("TH_DTMC_NSTEP_COND_PROB",
  ``!X p s (n:num) i j Linit Ltrans t.        
       th_dtmc X p s Linit Ltrans ==> 
       (Trans X p s t (n + 1) i j = 
        SIGMA (\k. Trans X p s t 1 k j * Trans X p s t n i k) (space s))``,
    RW_TAC std_ss []
 ...
 >> (Cases_on `i NOTIN (space s)`
    >> (`Trans X p s t 1 i j = 0` by RW_TAC std_ss [Trans_def]
       ++ FULL_SIMP_TAC arith_ss [REAL_MUL_RZERO]
       ++ (MP_TAC o Q.ISPEC `(space s):'b -> bool`) (GSYM REAL_SUM_IMAGE_0)
       ++ RW_TAC std_ss [Trans_def, REAL_MUL_RZERO])
     ++ Cases_on `j NOTIN (space s)`
     >> (`Trans X p s t 1 i j = 0` by RW_TAC std_ss [Trans_def]
       ++ FULL_SIMP_TAC arith_ss [REAL_MUL_RZERO]
       ++ (MP_TAC o Q.ISPEC `(space s):'b -> bool`) (GSYM REAL_SUM_IMAGE_0)
       ++ RW_TAC std_ss [Trans_def, REAL_MUL_LZERO]) 
     ++ `space s = {i} UNION (space s DIFF {i})` by PSET_TAC [UNION_DIFF]
     ++ POP_ORW ++ DEP_REWRITE_TAC [REAL_SUM_IMAGE_DISJOINT_UNION]
     ++ PSET_TAC [Trans_def, th_dtmc_def, FINITE_SING, FINITE_DIFF, DISJOINT_DIFF, 
     		REAL_SUM_IMAGE_SING, REAL_MUL_RID]
     ++ Suff `SIGMA (\k. (if k IN space s then
            cond_prob p (PREIMAGE (X (t + 1)) {j} INTER p_space p)
              (PREIMAGE (X t) {k} INTER p_space p) else 0) * 
              if k IN space s then if i = k then 1 else 0 else 0)
      			(space s DIFF {i}) = SIGMA (\x. 0) (space s DIFF {i})`
     >> (DEP_REWRITE_TAC [REAL_SUM_IMAGE_0,REAL_ADD_RID] 
        ++ RW_TAC std_ss [FINITE_DIFF, REAL_ADD_RID])   			
     ++ MATCH_MP_TAC REAL_SUM_IMAGE_EQ
     ++ PSET_TAC [FINITE_DIFF, REAL_MUL_RZERO])
 ++ PSET_TAC [ADD_ASSOC, th_dtmc_def]
 ++ Cases_on `i NOTIN (space s)`
 >> (`Trans X p s t (n + 1) i j = 0` by RW_TAC std_ss [Trans_def]
       ++ FULL_SIMP_TAC arith_ss [REAL_MUL_RZERO]
       ++ (MP_TAC o Q.ISPEC `(space s):'b -> bool`) (GSYM REAL_SUM_IMAGE_0)
       ++ RW_TAC std_ss [Trans_def, REAL_MUL_RZERO])
 ++ Cases_on `j NOTIN (space s)`
 >> (`Trans X p s t (n + 1) i j = 0` by RW_TAC std_ss [Trans_def]
    ...
    ++ (MP_TAC o Q.ISPEC `(space s):'b -> bool`) (GSYM REAL_SUM_IMAGE_0)
    ++ RW_TAC std_ss [Trans_def, REAL_MUL_LZERO]) 
 ++ RW_TAC std_ss [Trans_def, ADD_ASSOC]
 >> (`SIGMA (\k. (if k IN space s then
            cond_prob p (PREIMAGE (X (t + 1)) {j} INTER p_space p)
              (PREIMAGE (X t) {k} INTER p_space p) else 0) *
         if k IN space s then
           cond_prob p (PREIMAGE (X (t + n)) {k} INTER p_space p)
             (PREIMAGE (X t) {i} INTER p_space p) else 0) (space s) =
     SIGMA (\k. cond_prob p (PREIMAGE (X (t + 1)) {j} INTER p_space p)
    ...
    ++ DEP_REWRITE_TAC [REWRITE_RULE [COND_PMF_EQ_COND_PROB] MC_NSTEP_COND_PROB] 
    ++ RW_TAC std_ss [] 
    >> (MAP_EVERY Q.EXISTS_TAC [`Linit`, `Ltrans`] ++ RW_TAC std_ss [])
    ++ MATCH_MP_TAC REAL_SUM_IMAGE_EQ
    ++ RW_TAC std_ss []  
    ++ `cond_prob p (PREIMAGE (X (t + n + 1)) {j} INTER p_space p)
      (PREIMAGE (X (t + n)) {x} INTER p_space p) = Trans X p s (t + n) 1 x j` 
      by RW_TAC std_ss [Trans_def] ++ POP_ORW
    ++ `cond_prob p (PREIMAGE (X (t + 1)) {j} INTER p_space p)
      (PREIMAGE (X t) {x} INTER p_space p) = Trans X p s t 1 x j` 
      by RW_TAC std_ss [Trans_def] ++ POP_ORW
    ++ Suff `Trans X p s (t + n) 1 x j = Trans X p s t 1 x j`
    >> RW_TAC std_ss []
    ++ MATCH_MP_TAC HOMO_PROPERTY
    ++ MAP_EVERY Q.EXISTS_TAC [`Linit`, `Ltrans`] 
    ++ PSET_TAC [th_dtmc_def])
 ++ METIS_TAC []);

val HOMO_LEMMA = store_thm
  ("HOMO_LEMMA",
  ``!X p s Linit Ltrans.
	th_dtmc X p s Linit Ltrans ==>
 ...
 ++ `prob_space p` by PSET_TAC [th_dtmc_def, dtmc_def, mc_property_def, random_variable_def]    
 ++ Cases_on `i NOTIN (space s)`
 >> (`!t. PREIMAGE (X t) {i:'b} INTER p_space p = {}`
    	by (RW_TAC std_ss [] ++ MATCH_MP_TAC NOTIN_SPACE_EVENTS 
           ++ Q.EXISTS_TAC `s` ++ PSET_TAC [th_dtmc_def, dtmc_def, mc_property_def])
    ++ RW_TAC std_ss [cond_prob_def, INTER_EMPTY, PROB_EMPTY])
 ++ Cases_on `j NOTIN (space s)`
 >> (`!t. PREIMAGE (X (t + 1)) {j:'b} INTER p_space p = {}`
    	by (RW_TAC std_ss [] ++ MATCH_MP_TAC NOTIN_SPACE_EVENTS 
           ++ Q.EXISTS_TAC `s` ++ PSET_TAC [th_dtmc_def, dtmc_def, mc_property_def])
    ++ RW_TAC std_ss [cond_prob_def, INTER_EMPTY, PROB_EMPTY, REAL_DIV_LZERO]) 
 ++ `Trans X p s t 1 i j = Trans X p s n 1 i j` by METIS_TAC [HOMO_PROPERTY]
 ++ PSET_TAC [Trans_def]);
       	        
val THMC_TRANS_N = store_thm
  ("THMC_TRANS_N",
  ``!X p s t (m:num) (n:num) i j Linit Ltrans. 
	th_dtmc X p s Linit Ltrans /\ 0 < m ==> 
	(Trans X p s t m i j = Trans X p s n m i j)``,

    Induct_on `m` 
 >> RW_TAC std_ss [] 
 ++ RW_TAC std_ss [Trans_def, ADD1, ADD_ASSOC] 
 ++ (MP_TAC o Q.SPECL [`X`, `p`, `s`, `m`, `i`, `j`, `Linit`, `Ltrans`, `t`]) 
		TH_DTMC_NSTEP_COND_PROB ++ RW_TAC arith_ss [Trans_def]
 ++ (MP_TAC o Q.SPECL [`X`, `p`, `s`, `m`, `i`, `j`, `Linit`, `Ltrans`, `n`]) 
		TH_DTMC_NSTEP_COND_PROB ++ RW_TAC arith_ss [Trans_def]
 ++ NTAC 2 (POP_ASSUM K_TAC) 
 ++ `FINITE (space s)` by FULL_SIMP_TAC std_ss [th_dtmc_def]
 ++ MATCH_MP_TAC REAL_SUM_IMAGE_EQ 
 ++ RW_TAC arith_ss [REAL_MUL_RID, REAL_MUL_RZERO]
 << [METIS_TAC [HOMO_LEMMA],
     
     IMP_RES_TAC (SPEC_ALL (REWRITE_RULE [Trans_def] HOMO_LEMMA))
     ++ RW_TAC std_ss [] 
     ++ ONCE_ASM_REWRITE_TAC []
     ++ `!(a:real) b c. (a = b) ==> (c * a = c * b)` by RW_TAC std_ss []
     ++ POP_ASSUM MATCH_MP_TAC
     ++ FULL_SIMP_TAC arith_ss [Trans_def]
     ++ FIRST_ASSUM (MP_TAC o Q.SPECL [`X`, `p`, `s`, `t`, `n`, `i`, `i`, `Linit`, `Ltrans`])
     ++ RW_TAC std_ss [],
     ...
     ++ POP_ASSUM MATCH_MP_TAC
     ++ FULL_SIMP_TAC arith_ss [Trans_def]     
     ++ FIRST_ASSUM (MP_TAC o Q.SPECL [`X`, `p`, `s`, `t`, `n`, `i`, `x`, `Linit`, `Ltrans`])
     ++ RW_TAC std_ss []]);
   
val THMC_MNGT_CK_EQ = store_thm
  ("THMC_MNGT_CK_EQ",
  ``!X p s(m:num) (n:num) i j Linit Ltrans. 
	th_dtmc X p s Linit Ltrans /\ 0 < n /\ 0 < m ==> 
	(cond_pmf p (X (m + n)) (X 0) ({j},{i}) = 
	SIGMA (\k. cond_pmf p (X n) (X 0) ({j},{k}) * 
		cond_pmf p (X m) (X 0) ({k},{i})) (space s))``,

    Induct_on `n` 
 >> RW_TAC std_ss [] ++ RW_TAC std_ss [] ++ `m + SUC n = (m + n) + 1` by RW_TAC arith_ss [] 
 ...
 >> (RW_TAC std_ss [COND_PMF_EQ_COND_PROB] 
    ...
    ++ RW_TAC std_ss [cond_prob_def, INTER_EMPTY, real_div, PROB_EMPTY, 
    		REAL_DIV_LZERO, REAL_MUL_LZERO, REAL_MUL_RZERO, REAL_SUM_IMAGE_0])    		
 ++ Cases_on `prob p (PREIMAGE (X 0) {i} INTER p_space p) = 0`
 >> (RW_TAC std_ss [COND_PMF_EQ_COND_PROB, ADD1, ADD_ASSOC]
    ++ MP_RW_TAC COND_PROB_ZERO
    >> METIS_TAC [th_dtmc_def, DTMC_EVENTS, dtmc_def, mc_property_def, random_variable_def]
    ++ (MP_TAC o Q.ISPEC `(space s):'b -> bool`) (GSYM REAL_SUM_IMAGE_0)
    ++ RW_TAC std_ss [] ++ POP_ORW
    ++ MATCH_MP_TAC REAL_SUM_IMAGE_EQ 
    ++ RW_TAC std_ss []
    ++ Suff `cond_prob p (PREIMAGE (X m) {x} INTER p_space p)
 ...
 ++ Cases_on `n = 0` 
 >> (RW_TAC std_ss [COND_PMF_EQ_COND_PROB] 
    ++ `cond_prob p (PREIMAGE (X 0) {i} INTER p_space p)
           (PREIMAGE (X 0) {i} INTER p_space p) = 1`
           by (MATCH_MP_TAC COND_PROB_ITSELF 
              ++ METIS_TAC [th_dtmc_def, DTMC_EVENTS, dtmc_def, mc_property_def, 
              		random_variable_def, PROB_POSITIVE, REAL_LT_LE])
    ++ `SIGMA (\k. (if k IN space s then cond_prob p (PREIMAGE (X 1) {j} INTER p_space p)
                     (PREIMAGE (X 0) {k} INTER p_space p) else 0) *
                if k IN space s then
                  if m = 0 then if i = k then 1 else 0
                  else cond_prob p (PREIMAGE (X m) {k} INTER p_space p)
                      (PREIMAGE (X 0) {i} INTER p_space p) else 0) (space s) =
        SIGMA (\k. cond_prob p (PREIMAGE (X 1) {j} INTER p_space p)
                 (PREIMAGE (X 0) {k} INTER p_space p) *
		cond_prob p (PREIMAGE (X m) {k} INTER p_space p)
                   (PREIMAGE (X 0) {i} INTER p_space p)) (space s)`
	by (MATCH_MP_TAC REAL_SUM_IMAGE_EQ 
	   ++ RW_TAC std_ss [REAL_MUL_RZERO, REAL_MUL_RID]
	   >> RW_TAC std_ss [REAL_MUL_RID]
	   ++ Suff `cond_prob p (PREIMAGE (X 0) {x} INTER p_space p)
           		(PREIMAGE (X 0) {i} INTER p_space p) = 0`
	   >> RW_TAC std_ss [REAL_MUL_RZERO]
	   ++ MATCH_MP_TAC COND_PROB_DISJOINT_ZERO
	   ++ METIS_TAC [th_dtmc_def, DTMC_EVENTS, DISJOINT_PROC_INTER, dtmc_def,
	   	mc_property_def, random_variable_def]) 
    ++ (MP_TAC o Q.SPECL [`X`, `p`, `s`, `m`, `i`, `j`, `Linit`, `Ltrans`, `0`])
	 TH_DTMC_NSTEP_COND_PROB ++ RW_TAC std_ss [Trans_def])	 
 ++ (MP_TAC o Q.SPECL [`X`, `p`, `s`, `m + n`, `i`, `j`, `Linit`, `Ltrans`, `0`])
	 TH_DTMC_NSTEP_COND_PROB ++ RW_TAC std_ss [COND_PMF_EQ_COND_PROB, Trans_def] 
 >> (`SIGMA (\k. (if k IN space s then cond_prob p (PREIMAGE (X 1) {j} INTER p_space p)
              (PREIMAGE (X 0) {k} INTER p_space p) else 0) *
         if k IN space s then cond_prob p (PREIMAGE (X (m + n)) {k} INTER p_space p)
             (PREIMAGE (X 0) {i} INTER p_space p) else 0) (space s) =
      SIGMA (\k. cond_prob p (PREIMAGE (X 1) {j} INTER p_space p)
              (PREIMAGE (X 0) {k} INTER p_space p) *
         	cond_prob p (PREIMAGE (X (m + n)) {k} INTER p_space p)
             (PREIMAGE (X 0) {i} INTER p_space p)) (space s)` 
             by (MATCH_MP_TAC REAL_SUM_IMAGE_EQ ++ RW_TAC std_ss []) ++ POP_ORW
    ++ Know `SIGMA (\(k:'b). cond_pmf p (X (1:num)) (X 0) ({j},{k}) *
         cond_pmf p (X (m + n)) (X 0) ({k},{i})) (space s) =
         SIGMA (\k. if k IN (space s) then 
		SIGMA (\h. cond_pmf p (X 1) (X 0) ({j},{k}) * 
			   cond_pmf p (X n) (X 0) ({k},{h}) * 
			   cond_pmf p (X m) (X 0) ({h},{i})) (space s) else 0) (space s)` 
    >> (MATCH_MP_TAC REAL_SUM_IMAGE_EQ ++ RW_TAC std_ss [GSYM REAL_MUL_ASSOC] 
       ++ RW_TAC std_ss [(UNDISCH o Q.ISPEC `(space s):'b->bool`) REAL_SUM_IMAGE_CMUL]
       ++ Cases_on `cond_pmf p (X 1) (X 0) ({j},{x}) = 0` >> RW_TAC std_ss [REAL_MUL_LZERO]
       ++ RW_TAC std_ss [REAL_EQ_MUL_LCANCEL]
       ++ FIRST_X_ASSUM MATCH_MP_TAC 
       ++ MAP_EVERY Q.EXISTS_TAC [`Linit`, `Ltrans`]
       ++ RW_TAC arith_ss [])
    ++ RW_TAC std_ss [COND_PMF_EQ_COND_PROB] ++ POP_ASSUM K_TAC 
    ++ Know `!f. SIGMA (\k. if k IN space s then 
 			SIGMA (\a. f a k) (space s) else (0:real)) (space s) =
              SIGMA (\k. SIGMA (\a. f a k) (space s)) ((space s):'b->bool)` 
    >> ((MP_TAC o GSYM o Q.ISPEC `(space s):('b -> bool)`)
                 REAL_SUM_IMAGE_IN_IF ++ RW_TAC std_ss []
       ++ POP_ASSUM (MP_TAC o Q.SPEC `(\k. SIGMA (\a. f a k) ((space s):'b->bool))`) 
       ++ RW_TAC std_ss []) ++ RW_TAC std_ss []
    ++ Know `SIGMA (\(k:'b). cond_pmf p (X (n + (1:num))) (X 0) ({j},{k}) *
         cond_pmf p (X m) (X 0) ({k},{i})) (space s) = 
         SIGMA (\k. if k IN (space s) then SIGMA (\h. cond_pmf p (X 1) (X 0) ({j},{h}) * 
		cond_pmf p (X n) (X 0) ({h},{k}) * 
		cond_pmf p (X m) (X 0) ({k},{i})) (space s) else 0) (space s)`
    >> (MATCH_MP_TAC REAL_SUM_IMAGE_EQ ++ RW_TAC std_ss []
       ++ (MP_TAC o Q.SPECL [`X`, `p`, `s`, `(n:num)`, `(x:'b)`, `(j:'b)`, `Linit`, `Ltrans`, `0`]) 
		TH_DTMC_NSTEP_COND_PROB ++ RW_TAC std_ss [Trans_def, REAL_MUL_COMM]
       ++ `SIGMA (\k. (if k IN space s then cond_prob p (PREIMAGE (X n) {k} INTER p_space p)
                     (PREIMAGE (X 0) {x} INTER p_space p) else 0) *
         if k IN space s then cond_prob p (PREIMAGE (X 1) {j} INTER p_space p)
                    (PREIMAGE (X 0) {k} INTER p_space p) else 0) (space s) =
      	SIGMA (\k. cond_prob p (PREIMAGE (X n) {k} INTER p_space p)
                     (PREIMAGE (X 0) {x} INTER p_space p) *
         	cond_prob p (PREIMAGE (X 1) {j} INTER p_space p)
                    (PREIMAGE (X 0) {k} INTER p_space p)) (space s)` 
             by (MATCH_MP_TAC REAL_SUM_IMAGE_EQ ++ RW_TAC std_ss []) 
       ++ RW_TAC std_ss [COND_PMF_EQ_COND_PROB, (UNDISCH o Q.ISPEC `(space s):'b->bool`)
    	 REAL_SUM_IMAGE_CMUL])
    ++ RW_TAC std_ss [COND_PMF_EQ_COND_PROB,SWAP_SIGMA_SIGMA])
 ++ METIS_TAC []);

(* ------------------------------------------------------------------------- *)
(* Theorem 2: Chapman-Kolmogorov Equation                                    *)
(* --------------------------------------------------------------------------*) 
val THMC_CK_EQ = store_thm
  ("THMC_CK_EQ",
  ``!X p s t (m:num) (n:num) i j Linit Ltrans. 
	th_dtmc X p s Linit Ltrans ==> 
	(Trans X p s t (m + n) i j = 
	SIGMA (\k. Trans X p s t n k j * Trans X p s t m i k) (space s))``,
    RW_TAC std_ss []
 ++ `FINITE (space s)` by PSET_TAC [th_dtmc_def]
 ++ Cases_on `i NOTIN (space s)`
 >> RW_TAC std_ss [Trans_def, REAL_MUL_RZERO, REAL_SUM_IMAGE_0] 
 ++ Cases_on `j NOTIN (space s)`
 >> RW_TAC std_ss [Trans_def, REAL_MUL_LZERO, REAL_SUM_IMAGE_0]
 ++ Cases_on `n = 0`
 >> (PSET_TAC [Trans_def]
    << [`space s = {i} UNION (space s DIFF {i})` by PSET_TAC [UNION_DIFF]
        ++ POP_ORW ++ DEP_REWRITE_TAC [REAL_SUM_IMAGE_DISJOINT_UNION, REAL_SUM_IMAGE_SING]
        ++ `SIGMA (\k. (if k IN {i} UNION (space s DIFF {i}) then
            if k = i then 1 else 0 else 0) *
         	if k IN {i} UNION (space s DIFF {i}) then
           if i = k then 1 else 0 else 0) (space s DIFF {i}) =
         	SIGMA (\k. (0:real)) (space s DIFF {i})` 
         	by (MATCH_MP_TAC REAL_SUM_IMAGE_EQ ++ PSET_TAC [FINITE_DIFF, REAL_MUL_LZERO]) 
        ++ RW_TAC std_ss [FINITE_SING, FINITE_DIFF, DISJOINT_DIFF, REAL_MUL_LID,
     		REAL_MUL_LZERO, REAL_ADD_RID, REAL_SUM_IMAGE_0] ++ PSET_TAC [],
     		
      	`space s = {j} UNION (space s DIFF {j})` by PSET_TAC [UNION_DIFF]
        ++ POP_ORW ++ DEP_REWRITE_TAC [REAL_SUM_IMAGE_DISJOINT_UNION, REAL_SUM_IMAGE_SING]
        ++ PSET_TAC [FINITE_SING, FINITE_DIFF, DISJOINT_DIFF, 
     		REAL_MUL_LID, REAL_ADD_LID]
        ++ (MP_TAC o GSYM o Q.ISPEC `(space s) DIFF {j:'b}`) REAL_SUM_IMAGE_0 
        ++ RW_TAC std_ss [FINITE_DIFF]
        ++ POP_ORW ++ MATCH_MP_TAC REAL_SUM_IMAGE_EQ 
        ++ PSET_TAC [FINITE_DIFF, REAL_MUL_RID, REAL_SUM_IMAGE_0, REAL_MUL_LZERO],
        
        `space s = {j} UNION (space s DIFF {j})` by PSET_TAC [UNION_DIFF]
        ++ POP_ORW ++ DEP_REWRITE_TAC [REAL_SUM_IMAGE_DISJOINT_UNION, REAL_SUM_IMAGE_SING]
        ++ PSET_TAC [FINITE_SING, FINITE_DIFF, DISJOINT_DIFF, REAL_MUL_LID]
     	++ Suff `SIGMA (\k. (if (k = j) \/ k IN space s /\ k <> j then
            if k = j then 1 else 0 else 0) *
         if (k = j) \/ k IN space s /\ k <> j then
           cond_prob p (PREIMAGE (X (t + m)) {k} INTER p_space p)
             (PREIMAGE (X t) {i} INTER p_space p) else 0) (space s DIFF {j}) = 
             SIGMA (\x. 0) (space s DIFF {j})`
        >> (DEP_REWRITE_TAC [REAL_SUM_IMAGE_0] 
           ++ RW_TAC std_ss [FINITE_DIFF, REAL_ADD_RID])   			
        ++ MATCH_MP_TAC REAL_SUM_IMAGE_EQ
        ++ PSET_TAC [FINITE_DIFF, REAL_MUL_LZERO]])
 ++ Cases_on `m = 0`
 >> (PSET_TAC [Trans_def]
    ++ `space s = {i} UNION (space s DIFF {i})` by PSET_TAC [UNION_DIFF]
    ++ POP_ORW ++ DEP_REWRITE_TAC [REAL_SUM_IMAGE_DISJOINT_UNION, REAL_SUM_IMAGE_SING]
    ++ PSET_TAC [FINITE_SING, FINITE_DIFF, REAL_MUL_RID]
    ++ Suff `SIGMA (\k. (if (k = i) \/ k IN space s /\ k <> i then
            cond_prob p (PREIMAGE (X (t + n)) {j} INTER p_space p)
              (PREIMAGE (X t) {k} INTER p_space p) else 0) *
         if (k = i) \/ k IN space s /\ k <> i then
           if i = k then 1 else 0 else 0) (space s DIFF {i}) = 
             SIGMA (\x. 0) (space s DIFF {i})`
    >> (DEP_REWRITE_TAC [REAL_SUM_IMAGE_0] 
        ++ RW_TAC std_ss [FINITE_DIFF, REAL_ADD_RID])   			
    ++ MATCH_MP_TAC REAL_SUM_IMAGE_EQ
    ++ PSET_TAC [FINITE_DIFF, REAL_MUL_RZERO])     
 ++ PSET_TAC [Trans_def, ADD_ASSOC]
 ++ `SIGMA (\k. (if k IN space s then cond_prob p (PREIMAGE (X (t + n)) {j} INTER p_space p)
              (PREIMAGE (X t) {k} INTER p_space p) else 0) *
         if k IN space s then cond_prob p (PREIMAGE (X (t + m)) {k} INTER p_space p)
             (PREIMAGE (X t) {i} INTER p_space p) else 0) (space s) =
      	SIGMA (\k. cond_prob p (PREIMAGE (X (t + n)) {j} INTER p_space p)
                     (PREIMAGE (X t) {k} INTER p_space p) *
         	cond_prob p (PREIMAGE (X (t + m)) {k} INTER p_space p)
                    (PREIMAGE (X t) {i} INTER p_space p)) (space s)` 
             by (MATCH_MP_TAC REAL_SUM_IMAGE_EQ ++ RW_TAC std_ss []) ++ POP_ORW 
 ++ (MP_TAC o (REWRITE_RULE [COND_PMF_EQ_COND_PROB]) o 
     	Q.SPECL [`X`, `p`, `s`, `t + m`, `n`, `i`, `j`, `Linit`, `Ltrans`]) THMC_MNGT_CK_EQ
 ++ RW_TAC arith_ss [] ++ POP_ORW
 ++ (MP_TAC o (REWRITE_RULE [Trans_def]) o 
     	Q.SPECL [`X`, `p`, `s`, `t`, `m + n`, `0`, `i`, `j`, `Linit`, `Ltrans`]) 
     	THMC_TRANS_N
 ++ RW_TAC arith_ss []
 ++ Suff `SIGMA (\k. cond_prob p (PREIMAGE (X (n + t)) {j} INTER p_space p)
           (PREIMAGE (X t) {k} INTER p_space p) *
         cond_prob p (PREIMAGE (X (m + t)) {k} INTER p_space p)
           (PREIMAGE (X t) {i} INTER p_space p)) (space s) =
         ...
         cond_prob p (PREIMAGE (X m) {k} INTER p_space p)
           (PREIMAGE (X 0) {i} INTER p_space p)) (space s)` 
 >> (RW_TAC std_ss [] 
    ++ MATCH_MP_TAC (REWRITE_RULE [COND_PMF_EQ_COND_PROB] THMC_MNGT_CK_EQ)
    ++ MAP_EVERY Q.EXISTS_TAC [`Linit`, `Ltrans`]
    ++ RW_TAC arith_ss [])  
 ++ MATCH_MP_TAC REAL_SUM_IMAGE_EQ
 ++ PSET_TAC [th_dtmc_def]
 ++ (MP_TAC o (REWRITE_RULE [Trans_def]) o 
     	Q.SPECL [`X`, `p`, `s`, `t`, `m`, `0`, `i`, `x`, `Linit`, `Ltrans`]) THMC_TRANS_N
 ++ RW_TAC arith_ss [th_dtmc_def]
 ++ (MP_TAC o (REWRITE_RULE [Trans_def]) o 
     	Q.SPECL [`X`, `p`, `s`, `t`, `n`, `0`, `x`, `j`, `Linit`, `Ltrans`]) THMC_TRANS_N
 ++ RW_TAC arith_ss [th_dtmc_def]);
