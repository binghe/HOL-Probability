...
(* ========================================================================= *)
(*                 Conditional Probability developed by L. Liu		     *)
(* ========================================================================= *)
set_trace "Unicode" 0;
...
pred_setTheory measureTheory extra_pred_setTools extra_pred_setTheory arithmeticTheory realTheory 
realLib pairTheory transcTheory prim_recTheory extrealTheory probabilityTheory;
...
val THREE_SETS_INTER = store_thm
  ("THREE_SETS_INTER",
   ``!A B C. A INTER B INTER (C INTER B) = A INTER C INTER B``,
             ...
             `B INTER (C INTER B) = B INTER (B INTER C)` by RW_TAC std_ss [INTER_COMM] THEN
             ONCE_ASM_REWRITE_TAC [] THEN
             `B INTER (B INTER C) = B INTER C` 
                   by RW_TAC std_ss [GSYM INTER_ASSOC, INTER_IDEMPOT] THEN
             ONCE_ASM_REWRITE_TAC [] THEN
...
val COND_PROB_INTER_SPLIT = store_thm
 ...
 ++ RW_TAC std_ss [cond_prob_def, real_div, GSYM INTER_ASSOC, REAL_MUL_LINV, REAL_MUL_RID]);	   
...
app load ["HolKernel", "Parse", "boolLib", "bossLib", "arithmeticTheory", 
"polyTheory","realTheory", "realLib",  "jrhUtils", "seqTheory", "limTheory", 
"transcTheory", "listTheory", "pred_setTheory", "mesonLib", "boolTheory",
"probabilityTheory","measureTheory","util_probTheory","pred_setLib","numLib",
"extrealTheory","combinTheory","lebesgueTheory","real_sigmaTheory"];
...
(* ========================================================================= *)
(*            Binomial Coefficient developed by M. Elleuch     		     *)
(* ========================================================================= *)
...
g`!(a:real) (b:real) n. (a + b) pow n = a pow n +
 (&binomial n 1 * a pow (n - 1) * b pow 1) +
 sum (2,(n-1)) (\x. &binomial n x * a pow (n - x) * b pow x)`;

e(RW_TAC real_ss [EXP_PASCAL_REAL]);
e(RW_TAC real_ss [REAL_SUM_IMAGE_COUNT]);
e(KNOW_TAC (--`!n x. SUC n = SUC (SUC n - 1)`--));
e(ARITH_TAC);
e(DISCH_TAC THEN ONCE_ASM_REWRITE_TAC[]);
...
Define `Poisson_distr_rv X p (t:real) R n  = 
  ...
  /\ (distribution p X {&n} =  (((R * t) pow n) * (exp (- R * t) )) /((&FACT n):real) )`;



val trans_fun = Define ` trans_fun X p t R h n m = ( ?g1 g2 g3. 
...
val is_poisson_proces = Define `
    is_poisson_proces X p t R h n m=
    (real_random_variable (X) p) /\ trans_fun X p t R h n m`


val marcov_property = Define ` marcov_property X p i j n h =
(real_random_variable (\(t:real). X t) p) /\ ((prob p (BIGINTER (IMAGE 
(\k. PREIMAGE (\t. X t) {&k} INTER p_space p) (count n))) <> 0) ==> 
((cond_prob p  (PREIMAGE (\t. X (t+h)) {&j} INTER p_space p) (PREIMAGE
(\t. X t) {&i} INTER p_space p INTER BIGINTER (IMAGE (\k. PREIMAGE
...
val dist_fun = Define ` dist_fun X p t U h n m = ( ?g1 g2 g3. 
    cond_prob p (PREIMAGE (\t.X ((t:real) + h)) {((&n) :int) + &m} INTER p_space p) 
    (PREIMAGE (\t. X t) {&n} INTER p_space p) =  
    if (&m = 0) 
       then ((((U * t) pow n) * 
...
BD_trans_def  (X :real -> extreal) p t R h (i:num) j n = 
    if (j = i+1) 
       then CTMC_trans_def X p t (R i (i+1)) h (i+1) j n
    else  if (j = i-1)
       then CTMC_trans_def X p t (R i (i+1)) h (i-1) j n
    else  CTMC_trans_def X p t (R i i) h (i-1) j n`;
...
MM1_trans_def  (X :real -> extreal) p t R h (i:num) j n = 
    if (j = i+1) 
       then Poisson_Trans_Fun X p t h (i + 1) i R
    else  if (j = i-1)
...
val BDP_def = Define `BDP_def X p t n i j FUN IFUN R h s =
   BD_def X p t n i j FUN IFUN R h s /\
 MM1_trans_def X p t R h i j n `;


val Steady_state_eq = Define ` Steady_state_eq X n p f l =
(real_random_variable X p) /\ (prob p (PREIMAGE X {&n}
INTER p_space p) = ( 1 - f/l) * (f/l) pow n)`;
...
g` !n s p (t:real) h X. {&n} IN subsets s /\ {&n + 1} IN subsets s /\  
indep_rv p (\t. X (t + h)) (\t. X t) s s {&n + 1} {&n} 
 ==> (prob p ((PREIMAGE (\(t:real). X (t + h)) {&n + 1} INTER p_space p) INTER
       (PREIMAGE (\t. X t) {&n} INTER p_space p)) =
 (prob p (PREIMAGE (\t. X (t + h)) {&n + 1} INTER p_space p)
 * prob p (PREIMAGE (\t. X t) {&n} INTER p_space p))) `;
...
g`!X (R:real) p. {&n} IN subsets s /\ {&n + 1} IN subsets s /\ {&n + &m} IN subsets s /\
indep_rv p (\t. X (t + h)) (\t. X t) s s {&n} {&n} /\
indep_rv p (\t. X (t + h)) (\t.X (t)) s s {&n + 1} {&n}  /\
indep_rv p (\t. X (t + h)) (\t.X (t)) s s {&n + &m} {&n}  /\
...
(R > (0:real)) /\ (t > (0:real)) /\ 
   (h > 0)
/\ (!h. Poisson_distr_rv (\t.X (t + h)) p (t + h) R (n + 1))
/\ (!h. Poisson_distr_rv (\t.X (t + h)) p (t + h) R n) 
/\ (!h. Poisson_distr_rv (\t.X (t + h)) p (t + h) R (n + m)) 

    ==> 
  is_poisson_proces X p t R h n m`;

e(RW_TAC std_ss[ Poisson_distr_rv_def,is_poisson_proces]);
e (POP_ASSUM (MP_TAC o Q.SPEC `0:real`));
e(RW_TAC real_ss[ETA_THM]);
e(UNDISCH_TAC(-- ` !h.
             real_random_variable (\t. X (t + h)) p /\
             &(n + 1) IN IMAGE (\t. X (t + h)) (p_space p) /\
             (distribution p (\t. X (t + h)) {&(n + 1)} =
              (R * (t + h)) pow (n + 1) * exp (-R * (t + h)) /
              &FACT (n + 1)) `--));
e (DISCH_TAC);
e (POP_ASSUM (MP_TAC o Q.SPEC `0:real`));
e(RW_TAC real_ss[ETA_THM]);
...
e(RW_TAC real_ss[cond_prob,add_rzero]);    
...
vg`! (T: real) R n X p. (Poisson_distr_rv X p (T - (t:real)) R 0) /\ (T > t) ==> 
(Poisson_distr_rv X p T R n = (Poisson_distr_rv X p t R 0))`;


g`! (l: real) R n X p. (Poisson_distr_rv (\t. X t) p t R 0) /\ (l > t) /\ 
(Poisson_distr_rv (\t. X f) p f R 0) ==> ((distribution p (\t. X (f - t)) {&0}) = exp (- R * t))`;
...
Define `EXP_distr_rv X p (f:real)t R n  = 
     (real_random_variable X p)  /\  
     ( &n IN (IMAGE X (p_space p)))
  /\ (distribution p (\t. X ( f > t)) {&n} =  exp (- R * t) )`;
...
((cond_prob p (PREIMAGE (\t. X ( T > t + h )) {&0} INTER p_space p) 
      (PREIMAGE (\t. X T > t) {&0} INTER p_space p)) = 
(PREIMAGE (\t. X T > t) {&0} INTER p_space p))`;

e(RW_TAC std_ss[]);
e(RW_TAC std_ss[cond_prob]);    
e(MP_REWRITE_TAC INDEP_LEMMA3);
e(RW_TAC std_ss[]);
e(KNOW_TAC (--`!x y. (x:real) * y / y = x * (y/y)`--));
e(RW_TAC real_ss[real_div, REAL_MUL_ASSOC]);
e(DISCH_TAC THEN ASM_REWRITE_TAC[]);
...
g` !n s p (T:real) h X t. {&0} IN subsets s /\ indep_rv p (\t. X (T - t)) (\t. X t) s s {&0} {&0} 
 ==> (prob p ((PREIMAGE (\(t:real). X (T - t)) {&0} INTER p_space p) INTER
       (PREIMAGE (\t. X t) {&0} INTER p_space p)) =
 (prob p (PREIMAGE (\t. X (T - t)) {&0} INTER p_space p)
...
{&n} IN subsets s /\ {&n + 1} IN subsets s /\ 
 indep_rv1 p (X (t + h)) (X (t)) s s {&n + 1} {&n}  /\
(Poisson_distr_rv (X (t + h)) p (t + h) f k (n + 1))/\
prob p (PREIMAGE (X t) {&n} INTER p_space p)<> 0 /\
real_random_variable (X (t + h)) p /\
(!n. &n IN IMAGE (X (t + h)) (p_space p)) /\
~(f k pow n = 0)/\ ~(t = (0:real)) /\ 
  ~(f k = 0) /\ ~(h = 0) /\ ~(h = 0)  ==> 
      ?g. 
      (cond_prob p (PREIMAGE (X (t + h)) {&n + 1} INTER p_space p) 
      (PREIMAGE (X t) {&n} INTER p_space p) =(((f k * t) pow n) * 
      (exp (- f k * t)) / ((&FACT n):real))  * (f k * h + (g h)))`;
...
e(RW_TAC std_ss[REAL_FACT_NZ]);
e(DISCH_TAC THEN ASM_REWRITE_TAC[]);
e(` exp (-(f k * (t + h))) =  exp (-(f k * t + f k * h)) ` by RW_TAC std_ss[REAL_ADD_LDISTRIB]);
e(ASM_REWRITE_TAC[]);
e(RW_TAC std_ss[REAL_NEG_ADD]);
e(RW_TAC std_ss[EXP_ADD]);
e(RW_TAC std_ss[real_div]);
...
e(KNOW_TAC(--`!f k t h n g. ((f :'b -> real) (k :'b) * (t + h)) pow SUC n *
 exp (-(f k * t)) * exp (-(f k * h)) *  inv (&SUC n) =  
 exp (-(f k * t)) * (f k * (t + h)) pow SUC n  * exp (-(f k * h)) *
      inv (&SUC n) `--));
e(REPEAT GEN_TAC);
e(REAL_ARITH_TAC);
e(DISCH_TAC THEN ASM_REWRITE_TAC[]);
e(KNOW_TAC(--`!f k t h n g. ((f :'b -> real) (k :'b) * t) pow n * exp (-(f k * t)) * (f k * h + g h) =  
exp (-(f k * t)) * (f k * t) pow n * (f k * h + g h) `--));
e(REPEAT GEN_TAC);
e(REAL_ARITH_TAC);
e(DISCH_TAC THEN ASM_REWRITE_TAC[]);
e(REWRITE_TAC[GSYM REAL_MUL_ASSOC]);
e(MP_REWRITE_TAC (GSYM REAL_EQ_LMUL2));
e(RW_TAC real_ss[EXP_POS_LT, REAL_POS_NZ]);
e(KNOW_TAC(--`(f :'a -> real) (k :'a) * (t + h) = f k * t + f k * h `--));
e(RW_TAC real_ss[REAL_ADD_LDISTRIB]);
e(DISCH_TAC THEN ASM_REWRITE_TAC[]);
e(ONCE_REWRITE_TAC[BINOMIAL_DEFN]);
e(ONCE_REWRITE_TAC[SUM_DIFF]);
...
e(MP_REWRITE_TAC COMMON_OF_ADD_3);
e(RW_TAC std_ss[EXIST_OVER_AND]);
e(MATCH_MP_TAC REAL_MUL3_NZ);
e(RW_TAC std_ss[REAL_MUL2_NZ,POW_NZ,ADD1,GSYM REAL_OF_NUM_ADD]);
e(MATCH_MP_TAC REAL_POS_NZ);
e(MATCH_MP_TAC REAL_LET_ADD);;
e(RW_TAC real_ss[]);
e(RW_TAC std_ss[REAL_MUL_ASSOC]);
e(KNOW_TAC(--`&SUC n * (f k* t) pow n * f k * h *
      (f k * t * (f k * t) pow n / (&SUC n * (f k * t) pow n * f k * h) +
       1 +
       sum (2,n)
         (\x.
            &binomial (SUC n) x * (f k * t) pow (SUC n - x) *
            (f k * h) pow x) / (&SUC n * (f k * t) pow n * f k * h)) *
      exp (-(f k * t)) * exp (-(f k * h)) * inv (&SUC n) = 
     ((f k * t) pow n * f k * h *
      (f k * t * (f k * t) pow n / (&SUC n * (f k * t) pow n * f k * h) +
       1 +
       sum (2,n)
         (\x.
            &binomial (SUC n) x * (f k * t) pow (SUC n - x) *
            (f k * h) pow x) / (&SUC n * (f k * t) pow n * f k * h)) *
      exp (-(f k * t)) * exp (-(f k * h))) *((&SUC n)* inv (&SUC n))
...
e(RW_TAC real_ss[]);
e(RW_TAC real_ss[EXP_SPLIT]);
e(RW_TAC real_ss[REAL_ARITH ``(a:real)*b*c*d = a*(b*c*d)``]);
e(MP_REWRITE_TAC (GSYM REAL_EQ_LMUL2));;
e(RW_TAC std_ss[EXIST_OVER_AND]);
e(RW_TAC real_ss[REAL_MUL2_NZ,POW_NZ,REAL_ARITH `` (0 <> (a:real)) = (a <> 0)``]);
e(RW_TAC std_ss[REAL_MUL_ASSOC]);
e(RW_TAC std_ss[COMMON_OF_ADD_2,REAL_MUL2_NZ]);
e(EXISTS_TAC(--`(\h. (((f (k: 'b) * t * (f k * t) pow n / (&SUC n * (f k * t) pow n * f k * h) +
       1 +
       sum (2,n)
         (\x.
            &binomial (SUC n) x * (f k * t) pow (SUC n - x) *
            (f k * h) pow x) / (&SUC n * (f k * t) pow n * f k * h)) *
      (1 + lim (\n. sum (1,n) (\n. inv (&FACT n) * -(f k * h) pow n)))) - 1)*(f k * h))  `-- ));

e(RW_TAC std_ss[real_div]);;
e(REWRITE_TAC[REAL_ARITH ``1 + (a:real)*b*c = 1 + a*(b*c)``]);;
e(RW_TAC real_ss[REAL_MUL_RINV,REAL_MUL2_NZ]);
e(REAL_ARITH_TAC);;
val POISSON_LEMMA_2 = top_thm();
drop();
...
g`! (x:real). exp x = (@s. (\n. 1 + sum (1,n) (\n. inv (&FACT n) * x pow n)) --> s)`;
e(RW_TAC real_ss[exp]);
e(RW_TAC real_ss[suminf]);
e(RW_TAC real_ss[sums]);
e(KNOW_TAC (--`(\n. sum (0,n) (\n. inv (&FACT n) * x pow n))  = 
(\n. sum (0, SUC n - 1) (\n. inv (&FACT n) * x pow n)) `--));
e(RW_TAC real_ss[N_SHIFT]);
e (DISCH_TAC THEN ONCE_ASM_REWRITE_TAC []);
e(KNOW_TAC(--`!n. sum (0,SUC n - 1) (\n. inv (&FACT n) * x pow n) = 
((inv (&FACT 0) * x pow 0) + 
(sum (1,n - 1) (\n. inv (&FACT n) * x pow n)))`--));
e(KNOW_TAC(--`` ``--));
e(KNOW_TAC (--`  (\n. sum (0,SUC n - 1) (\n. inv (&FACT n) * x pow n)) 
...
((\n. (inv (&FACT 0) * x pow 0)) + 
(\n. sum (1,n - 1) (\n. inv (&FACT n) * x pow n)))`--));
 val is_poisson_pr = Define `
 is_poisson_pr X (r:real->real) k p (t:real) h =
 (real_random_variable (X) p) /\
 (!n m. ?g1 g2 g3. 
 cond_prob p (PREIMAGE (\t.X ((t:real) + h)) {&n + &m} INTER p_space p) 
 (PREIMAGE (\t. X t) {&n} INTER p_space p) =  
...
g` !p X h. (!t. t + h IN m_space p) /\
      	       real_random_variable X p  ==> 
               real_random_variable (\t. X (t + h)) p`;
e(RW_TAC std_ss[real_random_variable_def]);
e(POP_ASSUM MP_TAC);
e(POP_ASSUM MP_TAC);
e (POP_ASSUM (MP_TAC o Q.SPEC `t + h`));
e(RW_TAC std_ss[p_space_def]);
e(POP_ASSUM MP_TAC);
e(POP_ASSUM MP_TAC);
e (POP_ASSUM (MP_TAC o Q.SPEC `t + h`));
e(RW_TAC std_ss[p_space_def]);
 e(POP_ASSUM MP_TAC);
e(RW_TAC std_ss[IN_MEASURABLE_BOREL]);
r(1);
e(RW_TAC std_ss[space_def,subsets_def]);
e(MATCH_MP_TAC IN_MEASURABLE_BOREL_ADD
e(POP_ASSUM MP_TAC);
e(POP_ASSUM MP_TAC);
e(POP_ASSUM MP_TAC);
e(POP_ASSUM MP_TAC);
e (POP_ASSUM (MP_TAC o Q.SPEC `t`));
e(RW_TAC std_ss[]);
...
g`!p (X:real->num) (A1:real) (t:real). 
    (X(A1) = 1) /\ (counting_process X)/\ t < A1 ==> 
    (prob p (PREIMAGE X {&(X(t))}  INTER p_space p) = 
    (distribution p X {&0}))`;

 g`!X p (N:real->num) (A1:real) (t:real). 
    (N(A1) = 1) /\ (counting_process N)/\ t < A1 ==> 
    (prob p (PREIMAGE X {&(N(t))}  INTER p_space p) = 
    (distribution p X {&0}))`;
...
g` !N X p t R. (!n. Poisson_distr_rv X p t R n)/\ (N A1 = 1) /\
    counting_process N /\ t < A1 ==>  (prob p (PREIMAGE X {&N t} INTER
    p_space p) =  exp (-R * t))`;


e(RW_TAC real_ss[Poisson_distr_rv_def]); e(POP_ASSUM MP_TAC);
e(POP_ASSUM MP_TAC); e(POP_ASSUM MP_TAC); e(POP_ASSUM (MP_TAC o Q.SPEC
`0 :num `)); e(RW_TAC
real_ss[FACT,real_div,REAL_MUL_RINV,REAL_INV1]);;
e(REPEAT(POP_ASSUM MP_TAC) THEN  METIS_TAC[ INTERARRIVAL]);
...
g `!X pr. real_random_variable X pr  /\  
(IMAGE X (p_space  (prob_space pr X)) = {0;1}) ==>
           (expectation (prob_space pr X) X  = Normal pr)`;

e(REPEAT GEN_TAC);;
e(MP_REWRITE_TAC finite_expectation);;
e(RW_TAC real_ss[prob_space_def,p_space_def,m_space_def]);
e(RW_TAC std_ss [FINITE_INSERT, FINITE_SING,FINITE_SING]);
e(RW_TAC std_ss [FINITE_INSERT, FINITE_SING,EXTREAL_SUM_IMAGE_THM,FINITE_SING,mul_lzero,add_lzero]);
e(`{1} DELETE 0 = {1}` by EVAL_TAC);
e(ASM_REWRITE_TAC[]);;
e(RW_TAC std_ss [EXTREAL_SUM_IMAGE_SING]);
e(RW_TAC std_ss [mul_lone]);

val EXPEC = top_thm(); drop();


g`! X pr. (PMF X 1 (prob_space pr X) = pr)`;

e(RW_TAC std_ss[PMF_def,bern_rv_def,distribution_def]);
e(RW_TAC real_ss[PREIMAGE_def,INTER_DEF,Bern_space_def]);
e(RW_TAC real_ss[GSPECIFICATION]);
e(RW_TAC real_ss[PROB]);
e(RW_TAC real_ss[p_space_def,m_space_def]);
e(RW_TAC real_ss[mu_def]);
...
e(EVAL_TAC);
val EXPEC_X_Pr_POSINF = top_thm(); drop();
