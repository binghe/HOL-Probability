...
(*---------------------------*)
load "listTheory"; open listTheory;
fun SET_TAC L =
...
val real_def = Define `real x = (if ((x = NegInf) \/ (x = PosInf)) then 0:real else (@r. x = Normal r))`;
...
val extreal_mul_def = Define
  `(extreal_mul NegInf NegInf = PosInf) /\
   ...
   (extreal_mul (Normal x) NegInf = (if x = 0 then (Normal 0) else (if 0 < x then NegInf else PosInf))) /\
   (extreal_mul NegInf (Normal y) = (if y = 0 then (Normal 0) else (if 0 < y then NegInf else PosInf))) /\
   (extreal_mul (Normal x) PosInf = (if x = 0 then (Normal 0) else (if 0 < x then PosInf else NegInf))) /\
   (extreal_mul PosInf (Normal y) = (if y = 0 then (Normal 0) else (if 0 < y then PosInf else NegInf))) /\
...
val REAL_LT_LMUL_0_NEG = store_thm
 ...
 ++ METIS_TAC [REAL_MUL_LNEG,REAL_LT_IMP_LE,REAL_LE_MUL,REAL_NEG_GE0,REAL_NOT_LT]);
...
val REAL_LT_RDIV_EQ_NEG = store_thm
 ("REAL_LT_RDIV_EQ_NEG", ``!x y z:real. z < 0 ==> (y /z < x <=> x * z < y)``,
  RW_TAC real_ss []
...
val REAL_LE_RDIV_EQ_NEG = store_thm
 ("REAL_LE_RDIV_EQ_NEG", ``!x y z:real. z < 0 ==> (y /z <= x <=> x * z <= y)``,
  RW_TAC real_ss []
  ++ `0<-z` by RW_TAC real_ss [REAL_NEG_GT0]         
  ++ `z<>0` by (METIS_TAC [REAL_NEG_NZ])
  ++ EQ_TAC
...
("REAL_SUM_IMAGE_EQ_sum", ``!n r. sum (0,n) r = SIGMA r (count n)``,
  RW_TAC std_ss []
  ++ Induct_on `n`
...
val mul_rzero = store_thm
  ("mul_rzero",
   ``!x. x * 0 = 0``,
  Cases ++ RW_TAC real_ss [extreal_mul_def,extreal_of_num_def,REAL_MUL_RZERO]);

val mul_lzero = store_thm
  ("mul_lzero",
   ``!x. 0 * x = 0``,
  Cases ++ RW_TAC real_ss [extreal_mul_def,extreal_of_num_def,REAL_MUL_LZERO]);

val mul_rone = store_thm
  ("mul_rone",
   ``!x. x * 1 = x``,
  Cases ++ RW_TAC real_ss [extreal_mul_def,extreal_of_num_def,REAL_MUL_RID]);

val mul_lone = store_thm
  ("mul_lone",
   ``!x. 1 * x = x``,
  Cases ++ RW_TAC real_ss [extreal_mul_def,extreal_of_num_def,REAL_MUL_LID]);
...
("le_infty",``(!x. NegInf <= x /\ x <= PosInf) /\ (!x. (x <= NegInf) = (x = NegInf)) /\ (!x. (PosInf <= x) = (x = PosInf))``,
 RW_TAC std_ss [] ++  Cases_on `x` ++ RW_TAC std_ss [extreal_le_def]);
...
val le_01 = store_thm
("le_01",``0 <= 1``, RW_TAC std_ss [extreal_of_num_def,extreal_le_def,REAL_LE_01]);

val lt_01 = store_thm
("lt_01",``0 < 1``, METIS_TAC [extreal_of_num_def,extreal_lt_def,extreal_le_def,REAL_LT_01,real_lt]);

val ne_01 = store_thm
("ne_01",``0 <> 1``, RW_TAC std_ss [extreal_of_num_def,REAL_10]);

val le_num = store_thm
("le_num",``!n:num. 0 <= &n``, RW_TAC real_ss [extreal_of_num_def,extreal_le_def]);
...
val le_neg = store_thm
 ("le_neg",``!x y. -x <= -y <=> y <= x``,
  Cases ++ Cases
  ++ RW_TAC std_ss [extreal_lt_eq,extreal_le_def,extreal_ainv_def,lt_infty,le_infty,REAL_LE_NEG]);

val lt_neg = store_thm
 ("lt_neg",``!x y. -x < -y <=> y < x``,
  Cases ++ Cases
  ++ RW_TAC std_ss [extreal_lt_eq,extreal_le_def,extreal_ainv_def,lt_infty,le_infty,REAL_LT_NEG]);
...
val let_add2 = store_thm
  ("let_add2",``!w x y z. w <> NegInf /\ w <> PosInf /\  y <> NegInf /\ y <> PosInf /\ w <= x /\ y < z ==> w + y < x + z``,
  Cases ++ Cases ++ Cases ++ Cases
...
val le_addr = store_thm
  ...
  REPEAT Cases
  ++ RW_TAC std_ss [extreal_add_def,extreal_le_def,le_infty,extreal_of_num_def,extreal_not_infty,REAL_LE_ADDR]);
...
val lt_addl = store_thm
  ...
  REPEAT Cases
  ++ RW_TAC std_ss [extreal_add_def,extreal_lt_def,extreal_le_def,le_infty,extreal_of_num_def,extreal_not_infty]
  ++ METIS_TAC [REAL_ADD_COMM,REAL_LE_SUB_LADD,real_sub,REAL_NEG_GE0,REAL_LE_ADDL]);

val le_lneg = store_thm
  ("le_lneg",``!x y. ((x <> NegInf /\ y <> NegInf) \/ (x <> PosInf /\ y <> PosInf)) ==> ((-x <= y) <=> (0 <= x + y))``,
  REPEAT Cases
  ++ RW_TAC std_ss [extreal_ainv_def,extreal_le_def,extreal_add_def,extreal_sub_def,le_infty,extreal_of_num_def,extreal_not_infty,REAL_LE_LNEG]);

val le_mul = store_thm
("le_mul",``!x y:extreal. 0 <= x /\ 0 <= y ==> 0 <= x * y``,
  Cases ++ Cases
  ++ RW_TAC std_ss [extreal_le_def,extreal_mul_def,extreal_of_num_def,REAL_LE_MUL,GSYM real_lt]
...
("add_comm",``!x y:extreal. (x <> NegInf /\ y <> NegInf) \/ (x <> PosInf /\ y <> PosInf) ==> (x + y = y + x)``,
  Cases ++ Cases_on `y`
  ++ RW_TAC std_ss [extreal_add_def,REAL_ADD_COMM]);

val add_comm_normal = store_thm
...
("EXTREAL_EQ_LADD",``!x y z. x <> NegInf /\ x <> PosInf ==> ((x + y = x + z) = (y = z))``,
  Cases ++ Cases ++ Cases
...
val sub_lzero = store_thm
  ("sub_lzero", ``!x. 0 - x = -x``,
  Cases ++ METIS_TAC [extreal_ainv_def,extreal_sub_def,extreal_of_num_def,REAL_SUB_LZERO]);

val sub_le_imp = store_thm
  ("sub_le_imp",``!x y z. x <> NegInf /\ x <> PosInf /\ y <= z + x ==> y - x <= z``,
   REPEAT Cases
   ++ RW_TAC std_ss [extreal_le_def,extreal_add_def,extreal_sub_def,REAL_LE_SUB_RADD]);

val sub_le_imp2 = store_thm
  ("sub_le_imp2",``!x y z. y <> NegInf /\ y <> PosInf /\ y <= z + x ==> y - x <= z``,
   REPEAT Cases
   ++ RW_TAC std_ss [extreal_le_def,extreal_add_def,extreal_sub_def,REAL_LE_SUB_RADD]);
...
("sub_zero_lt", ``!x y. x < y ==> 0 < y - x``,
  REPEAT Cases 
  ++ RW_TAC real_ss [extreal_le_def,extreal_add_def,extreal_sub_def,extreal_lt_eq,lt_infty,extreal_of_num_def,extreal_not_infty,REAL_SUB_LT]);
...
("sub_lt_zero", ``!x y. x < y ==> x - y < 0``,
  REPEAT Cases 
  ++ RW_TAC real_ss [extreal_le_def,extreal_add_def,extreal_sub_def,extreal_lt_eq,lt_infty,extreal_of_num_def,extreal_not_infty,REAL_LT_SUB_RADD]);
...
("sub_zero_le", ``!x y. x <> NegInf /\ x <> PosInf ==> (x <= y = 0 <= y - x)``,
  REPEAT Cases 
  ++ RW_TAC real_ss [extreal_le_def,extreal_add_def,extreal_sub_def,lt_infty,extreal_of_num_def,extreal_not_infty,REAL_SUB_LE]);

val sub_le_zero = store_thm
("sub_le_zero", ``!x y. y <> NegInf /\ y <> PosInf ==> (x <= y = x - y <= 0)``,
  REPEAT Cases 
  ++ RW_TAC real_ss [extreal_le_def,extreal_add_def,extreal_sub_def,lt_infty,extreal_of_num_def,extreal_not_infty,REAL_LE_SUB_RADD]);
...
val le_sub_eq2 = store_thm
 ("le_sub_eq2",``!x y z. z <> NegInf /\ z <> PosInf /\ x <> NegInf /\ y <> NegInf ==>
       (y <= z - x = y + x <= z)``,
  REPEAT Cases 
  ++ RW_TAC real_ss [extreal_le_def,extreal_add_def,extreal_sub_def,lt_infty,extreal_of_num_def,extreal_not_infty,REAL_LE_SUB_LADD]);

val sub_le_eq = store_thm
  ("sub_le_eq",``!x y z. x <> NegInf /\ x <> PosInf ==> (y - x <= z = y <= z + x)``,
...
val sub_le_switch = store_thm
  ("sub_le_switch",``!x y z. x <> NegInf /\ x <> PosInf /\ z <> NegInf /\ z <> PosInf ==> (y - x <= z = y - z <= x)``,
  Cases ++ Cases ++ Cases
  ++ RW_TAC std_ss [extreal_sub_def,extreal_le_def,le_infty,lt_infty]
  ++ REAL_ARITH_TAC);

val sub_le_switch2 = store_thm
  ("sub_le_switch2",``!x y z. x <> NegInf /\ x <> PosInf /\ y <> NegInf /\ y <> PosInf ==> (y - x <= z = y - z <= x)``,
  Cases ++ Cases ++ Cases
  ++ RW_TAC std_ss [extreal_sub_def,extreal_le_def,le_infty,lt_infty]
  ++ REAL_ARITH_TAC);

val lt_sub = store_thm
  ("lt_sub",``!x y z. x <> NegInf /\ y <> NegInf /\ z <> NegInf /\ z <> PosInf ==> (y + x < z = y < z - x)``,
 REPEAT Cases
 ++ RW_TAC std_ss [extreal_lt_def,extreal_le_def,extreal_add_def,extreal_sub_def,le_infty]
...
val add_sub = store_thm
 ...
 REPEAT Cases
 ++ RW_TAC std_ss [extreal_lt_def,extreal_le_def,extreal_add_def,extreal_sub_def,REAL_ADD_SUB_ALT]);

val add_sub2 = store_thm
  ("add_sub2",``!x y. y <> NegInf /\ y <> PosInf ==> (y + x - y = x)``,
 REPEAT Cases
 ++ RW_TAC std_ss [extreal_lt_def,extreal_le_def,extreal_add_def,extreal_sub_def,REAL_ADD_SUB]);

val sub_add = store_thm
  ("sub_add",``!x y. y <> NegInf /\ y <> PosInf ==> ((x - y) + y = x)``,
 REPEAT Cases
 ++ RW_TAC std_ss [extreal_lt_def,extreal_le_def,extreal_add_def,extreal_sub_def,REAL_SUB_ADD]);

val extreal_sub_add = store_thm
  ("extreal_sub_add",``!x y. (x <> NegInf /\ y <> PosInf) \/ (x <> PosInf /\ y <> NegInf) ==> (x - y = x + -y)``,
 REPEAT Cases
...
val sub_0 = store_thm
  ("sub_0",``!x y. (x - y = 0) ==> (x = y)``,
 REPEAT Cases
 ++ RW_TAC std_ss [extreal_sub_def,num_not_infty,REAL_SUB_0,extreal_of_num_def,extreal_11]);
...
val sub_not_infty = store_thm
  ...
  Cases ++ Cases ++ RW_TAC std_ss [extreal_sub_def]);
...
("mul_assoc",``!x y z:extreal. x * (y * z) = x * y * z``,
  Cases ++ Cases ++ Cases
  ++ RW_TAC real_ss [extreal_mul_def,REAL_MUL_ASSOC,REAL_MUL_LZERO,REAL_MUL_RZERO,REAL_ENTIRE,REAL_LT_LMUL_0,REAL_POS_NZ,DE_MORGAN_THM]
...
val add_ldistrib_neg_neg = store_thm
...
val add_ldistrib_pos_pos = store_thm
...
val le_rdiv = store_thm
  ...
  ++ RW_TAC std_ss [extreal_mul_def,extreal_div_def,extreal_inv_def,extreal_le_def,le_infty,extreal_of_num_def,extreal_not_infty,REAL_LT_REFL,REAL_INV_EQ_0,REAL_INV_POS]
...
val lt_rdiv = store_thm
 ("lt_rdiv", ``!x y z. 0 < z ==> (x < y / Normal z <=> x * Normal z < y)``,
  Cases ++ Cases 
  ++ RW_TAC std_ss [REAL_INV_EQ_0,REAL_INV_POS,extreal_lt_eq,REAL_LT_RDIV_EQ,GSYM real_div,REAL_LT_REFL,lt_refl,lt_infty,le_infty,extreal_div_def,extreal_inv_def,extreal_div_eq,extreal_mul_def]);

val lt_ldiv = store_thm
  ("lt_ldiv",``!x y z. 0 < z ==> (x / Normal z < y = x < y * Normal z)``,
  Cases ++ Cases
  ++ RW_TAC std_ss [REAL_INV_EQ_0,REAL_INV_POS,extreal_lt_eq,REAL_LT_LDIV_EQ,GSYM real_div,REAL_LT_REFL,lt_refl,lt_infty,le_infty,extreal_div_def,extreal_inv_def,extreal_div_eq,extreal_mul_def]);

val lt_rdiv_neg = store_thm
 ("lt_rdiv_neg", ``!x y z. z < 0 ==> (y / Normal z < x <=> x * Normal z < y)``,
  Cases ++ Cases 
  ++ RW_TAC std_ss [REAL_INV_POS,REAL_LE_LT,GSYM real_lte,REAL_INV_EQ_0,REAL_INV_POS,extreal_lt_eq,REAL_LT_RDIV_EQ_NEG,GSYM real_div,REAL_LT_REFL,lt_refl,lt_infty,le_infty,extreal_div_def,extreal_inv_def,extreal_div_eq,extreal_mul_def]
  ++ `inv z = - inv (-z)` by METIS_TAC [REAL_NEG_INV,REAL_NEG_NEG] 
  ++ `0 < -z` by METIS_TAC [REAL_NEG_GT0]
  ++ `0 < inv (-z)` by METIS_TAC [REAL_INV_POS]
  ++ `inv z < 0` by METIS_TAC [REAL_NEG_GT0,REAL_NEG_NEG]
  ++ `-inv (-z) < 0` by METIS_TAC [REAL_NEG_LT0]
  ++ METIS_TAC [REAL_LT_ANTISYM,lt_refl,lt_infty]);

(*
val div_add = store_thm
  ("div_add",``!x y z. 0 < z ==> (x / Normal z < y = x < y * Normal z)``,
  Cases ++ Cases
  ++ RW_TAC std_ss [REAL_INV_EQ_0,REAL_INV_POS,extreal_lt_eq,REAL_LT_LDIV_EQ,GSYM real_div,REAL_LT_REFL,lt_refl,lt_infty,le_infty,extreal_div_def,extreal_inv_def,extreal_div_eq,extreal_mul_def]);


g `!x y z. x <> NegInf /\ y <> NegInf ==> (x / z + y / z = (x + y) / z)`;


  REPEAT Cases
  ++ RW_TAC real_ss [extreal_add_def,extreal_div_def,extreal_mul_def,extreal_inv_def]
  << [Cases_on `inv r = 0` >> RW_TAC real_ss [extreal_add_def,extreal_11]
      RW_TAC real_ss [extreal_add_def],
...
val pow_zero = store_thm
 ("pow_zero",``!n x. (x pow (SUC n) = 0) = (x = 0)``,
  STRIP_TAC ++ Cases ++ RW_TAC std_ss [extreal_pow_def,extreal_of_num_def,POW_ZERO_EQ]);

val pow_zero_imp = store_thm
 ("pow_zero_imp",``!n x. (x pow n = 0) ==> (x = 0)``,
  STRIP_TAC ++ Cases ++ RW_TAC std_ss [extreal_pow_def,extreal_of_num_def,REAL_LT_01,REAL_LT_IMP_NE]
  ++ METIS_TAC [POW_ZERO]);

val le_pow2 = store_thm
 ("le_pow2",``!x. 0 <= x pow 2``,
  Cases ++ RW_TAC std_ss [extreal_pow_def,extreal_of_num_def,extreal_le_def,REAL_LE_POW2]);
...
val pow_pos_lt = store_thm
 ("pow_pos_lt",``!x n. 0 < x ==> 0 < x pow n``,
  Cases ++ Cases ++ RW_TAC std_ss [extreal_pow_def,extreal_of_num_def,extreal_le_def,extreal_lt_eq,POW_POS_LT,REAL_LT_01,lt_infty,extreal_not_infty]
...
val add_pow2 = store_thm
 ("add_pow2",``!x y. x <> NegInf /\ x <> PosInf /\ y <> NegInf /\ y <> PosInf ==> 
...
val pow_add = store_thm
 ("pow_add",``!x n m. x pow (n + m) = x pow n * x pow m``, 
  Cases ++ RW_TAC real_ss [extreal_pow_def,POW_ADD,extreal_of_num_def,extreal_mul_def,mul_rone,mul_lone]
...
val pow_mul = store_thm
 ("pow_mul",``!n x y. (x * y) pow n = x pow n * y pow n``,
  ...
  ++ Cases ++ Cases 
  ++ RW_TAC real_ss [extreal_mul_def,extreal_pow_def,pow_zero,POW_ZERO_EQ,POW_POS_LT,POW_MUL]
  ++ FULL_SIMP_TAC real_ss [GSYM real_lte]
  ++ METIS_TAC [POW_POS_EVEN,POW_NEG_ODD,REAL_LT_LE,POW_POS_LT,real_lt,POW_ZERO_EQ,EVEN_ODD]);
...
val pow_not_infty = store_thm
 ("pow_not_infty",``!n x. x <> NegInf /\ x <> PosInf ==> x pow n <> NegInf /\ x pow n <> PosInf``,
...
val sqrt_pos_lt = store_thm
 ("sqrt_pos_lt",``!x. 0 < x ==> 0 < sqrt x``,
  Cases ++ RW_TAC std_ss [extreal_sqrt_def,extreal_of_num_def,extreal_le_def,extreal_lt_eq,lt_infty,SQRT_POS_LT]);

val pow2_sqrt = store_thm
 ("pow2_sqrt",``!x. 0 <= x ==> (sqrt (x pow 2) = x)``,
  Cases ++ RW_TAC real_ss [extreal_sqrt_def,extreal_pow_def,POW_2_SQRT,extreal_of_num_def,extreal_le_def]);

val sqrt_pow2 = store_thm
 ("sqrt_pow2",``!x. (sqrt x pow 2 = x) <=> 0 <= x``,
  Cases ++ RW_TAC real_ss [extreal_sqrt_def,extreal_pow_def,SQRT_POW2,extreal_of_num_def,extreal_le_def]
...
val EXTREAL_ARCH_POW = store_thm
  ...
  Cases ++ RW_TAC std_ss [lt_infty,extreal_lt_eq,REAL_ARCH_POW,extreal_pow_def,extreal_of_num_def]);
...
val EXTREAL_ARCH_POW_INV = store_thm
  ...
  ++ RW_TAC std_ss [REAL_SUB_RZERO, REAL_POW_LT,REAL_HALF_BETWEEN,REAL_LT_IMP_LE,
                      GREATER_EQ]
...
val REAL_LE_MUL_EPSILON = store_thm
  ("REAL_LE_MUL_EPSILON",
   ``!x y:real. (!z. 0 < z /\ z < 1 ==> z * x <= y) ==>
    ...
    REPEAT STRIP_TAC
    ++ Cases_on `x = 0`
    >> (Q.PAT_ASSUM `!z. P z` (MP_TAC o Q.SPEC `1/2`) ++ fs[])
    ++ Cases_on `0 < x`
...
val le_epsilon = store_thm
  ...
  << [Q.EXISTS_TAC `1` ++ RW_TAC std_ss [lt_01,extreal_of_num_def,extreal_not_infty,extreal_add_def],
      Q.EXISTS_TAC `1` ++ RW_TAC std_ss [lt_01,extreal_of_num_def,extreal_not_infty,extreal_add_def],
      Q.EXISTS_TAC `1` ++ RW_TAC std_ss [lt_01,extreal_of_num_def,extreal_not_infty,extreal_add_def,extreal_le_def],
      ...
      ++ `!e. 0 < e ==> Normal r <= Normal (r' + e)`
         by (RW_TAC real_ss [extreal_le_def,REAL_LT_IMP_LE,REAL_LE_ADD]
             ...
             ++ `Normal r' + Normal e = Normal (r' + e)` by METIS_TAC [extreal_add_def,REAL_LT_IMP_LE]
             ++ FULL_SIMP_TAC std_ss []
...
("le_mul_epsilon",``!x y:extreal. (!z. 0 <= z /\ z < 1 ==> z * x <= y) ==> x <= y``,
  ...
  << [Q.EXISTS_TAC `1 / 2` 
      ...
      Q.EXISTS_TAC `1 / 2` 
      ++ RW_TAC real_ss [extreal_mul_def,extreal_of_num_def,extreal_div_eq,extreal_cases,le_infty,extreal_not_infty],
      Q.EXISTS_TAC `1 / 2` 
      ++ RW_TAC real_ss [extreal_mul_def,extreal_of_num_def,extreal_div_eq,extreal_cases,le_infty,extreal_not_infty],
...
val EXTREAL_SUM_IMAGE_IND = fetch "-" "EXTREAL_SUM_IMAGE_ind";

val EXTREAL_SUM_IMAGE_THM = W (GENL o rev o free_vars o concl)
  (DISCH_ALL(ASM_REWRITE_RULE [ASSUME ``FINITE s``] EXTREAL_SUM_IMAGE_def));

val EXTREAL_SUM_IMAGE_EMPTY = REWRITE_RULE [FINITE_EMPTY]
  (SPEC ``{}`` EXTREAL_SUM_IMAGE_THM);

val _ = save_thm("EXTREAL_SUM_IMAGE_IND",EXTREAL_SUM_IMAGE_IND);
val _ = save_thm("EXTREAL_SUM_IMAGE_THM",EXTREAL_SUM_IMAGE_THM);
val _ = save_thm("EXTREAL_SUM_IMAGE_EMPTY",EXTREAL_SUM_IMAGE_EMPTY);

val EXTREAL_SUM_IMAGE_INSERT = Q.store_thm
("EXTREAL_SUM_IMAGE_INSERT",
 `!s. FINITE s ==>
        !f x. EXTREAL_SUM_IMAGE f (x INSERT s) =
               f (CHOICE (x INSERT s)) +  EXTREAL_SUM_IMAGE f (REST (x INSERT s))`,
  REPEAT STRIP_TAC THEN
  POP_ASSUM (fn th =>
    `FINITE (x INSERT s)` by PROVE_TAC [th, FINITE_INSERT]) THEN
  GEN_REWR_TAC LAND_CONV [EXTREAL_SUM_IMAGE_THM] THEN
...
("EXTREAL_SUM_IMAGE_NOT_NEG_INF",``!f s. FINITE s /\ (!x. x IN s ==> f x <> NegInf) ==>
  ...
  ++ Induct_on `CARD s`
  >> METIS_TAC [CARD_EQ_0,EXTREAL_SUM_IMAGE_THM,extreal_of_num_def,extreal_not_infty]
  ...
  ++ FULL_SIMP_TAC std_ss [EXTREAL_SUM_IMAGE_INSERT,FINITE_INSERT]
...
("EXTREAL_SUM_IMAGE_PROPERTY_NEG",``!f s. FINITE s ==>
          !e. (!x. x IN e INSERT s ==> f x <> NegInf) ==> (EXTREAL_SUM_IMAGE f (e INSERT s) = f e + EXTREAL_SUM_IMAGE f (s DELETE e))``,
  REPEAT GEN_TAC ++ STRIP_TAC
  ++ completeInduct_on `CARD s`
  ++ POP_ASSUM (ASSUME_TAC o SIMP_RULE bool_ss
        [GSYM RIGHT_FORALL_IMP_THM, AND_IMP_INTRO]) 
  ++ GEN_TAC 
  ++ SIMP_TAC bool_ss [EXTREAL_SUM_IMAGE_INSERT]
  ++ REPEAT STRIP_TAC
  ++ Q.ABBREV_TAC `t = REST (e INSERT s)`
  ++ Q.ABBREV_TAC `y = CHOICE (e INSERT s)` 
  ...
  << [FULL_SIMP_TAC bool_ss [absorption]
      ...
      << [POP_ASSUM SUBST_ALL_TAC
          ...
          `s = y INSERT t` by PROVE_TAC [NOT_IN_EMPTY, CHOICE_INSERT_REST]
          ++ `e IN t` by PROVE_TAC [IN_INSERT]
          ++ Q.ABBREV_TAC `u = t DELETE e`
          ++ `t = e INSERT u` by SRW_TAC [][INSERT_DELETE, Abbr`u`]
          ++ `~(e IN u)` by PROVE_TAC [IN_DELETE]
          ++ `s = e INSERT (y INSERT u)` by SRW_TAC [][INSERT_COMM]
          ++ POP_ASSUM SUBST_ALL_TAC
          ++ FULL_SIMP_TAC bool_ss [FINITE_INSERT, CARD_INSERT, DELETE_INSERT, IN_INSERT]
	  ++ `CARD u < SUC (SUC (CARD u))` by RW_TAC arith_ss []
	  ++ `EXTREAL_SUM_IMAGE f (e INSERT u) = f e + EXTREAL_SUM_IMAGE f u` by METIS_TAC [delete_non_element]
          ++ `EXTREAL_SUM_IMAGE f (y INSERT u) = f y + EXTREAL_SUM_IMAGE f u` by METIS_TAC [delete_non_element]
	  ++ `EXTREAL_SUM_IMAGE f u <> NegInf` by METIS_TAC [EXTREAL_SUM_IMAGE_NOT_INFTY]
	  ++ METIS_TAC [add_assoc,add_comm]],
      ASM_SIMP_TAC bool_ss [delete_non_element]
      ++ `e INSERT s = y INSERT t`
         by PROVE_TAC [NOT_EMPTY_INSERT, CHOICE_INSERT_REST]
      ++ Cases_on `e = y`
      << [POP_ASSUM SUBST_ALL_TAC
          ++ Q_TAC SUFF_TAC `t = s` >> SRW_TAC [][]
          ++ FULL_SIMP_TAC bool_ss [EXTENSION, IN_INSERT] 
          ++ PROVE_TAC [],   
	  `e IN t /\ y IN s` by PROVE_TAC [IN_INSERT]
	  ...
	  ++ `CARD u < SUC (CARD u)` by RW_TAC arith_ss []
	  ++ `EXTREAL_SUM_IMAGE f (e INSERT u) = f e + EXTREAL_SUM_IMAGE f u` by METIS_TAC [delete_non_element]
          ++ `EXTREAL_SUM_IMAGE f (y INSERT u) = f y + EXTREAL_SUM_IMAGE f u` by METIS_TAC [delete_non_element]
          ++ `EXTREAL_SUM_IMAGE f u <> NegInf` by METIS_TAC [EXTREAL_SUM_IMAGE_NOT_INFTY]
          ++ METIS_TAC [add_assoc,add_comm]]]);
...
("EXTREAL_SUM_IMAGE_PROPERTY_POS",``!f s. FINITE s ==>
          !e. (!x. x IN e INSERT s ==> f x <> PosInf) ==> (EXTREAL_SUM_IMAGE f (e INSERT s) = f e + EXTREAL_SUM_IMAGE f (s DELETE e))``,
  REPEAT GEN_TAC ++ STRIP_TAC
  ++ completeInduct_on `CARD s`
  ++ POP_ASSUM (ASSUME_TAC o SIMP_RULE bool_ss
        [GSYM RIGHT_FORALL_IMP_THM, AND_IMP_INTRO]) 
  ...
  << [FULL_SIMP_TAC bool_ss [absorption]
	  ...
	  ++ `EXTREAL_SUM_IMAGE f (e INSERT u) = f e + EXTREAL_SUM_IMAGE f u` by METIS_TAC [delete_non_element]
          ++ `EXTREAL_SUM_IMAGE f (y INSERT u) = f y + EXTREAL_SUM_IMAGE f u` by METIS_TAC [delete_non_element]
	  ++ `EXTREAL_SUM_IMAGE f u <> PosInf` by METIS_TAC [EXTREAL_SUM_IMAGE_NOT_INFTY]
	  ++ METIS_TAC [add_assoc,add_comm]],
      ASM_SIMP_TAC bool_ss [delete_non_element]
      ++ `e INSERT s = y INSERT t`
         by PROVE_TAC [NOT_EMPTY_INSERT, CHOICE_INSERT_REST]
...
("EXTREAL_SUM_IMAGE_PROPERTY",``!f s. FINITE s  ==>
          !e. (!x. x IN e INSERT s ==> f x <> NegInf) \/ (!x. x IN e INSERT s ==> f x <> PosInf) ==> (EXTREAL_SUM_IMAGE f (e INSERT s) = f e + EXTREAL_SUM_IMAGE f (s DELETE e))``,
  METIS_TAC [EXTREAL_SUM_IMAGE_PROPERTY_NEG,EXTREAL_SUM_IMAGE_PROPERTY_POS]);

val EXTREAL_SUM_IMAGE_SING = store_thm(
  "EXTREAL_SUM_IMAGE_SING",
  ``!f e. EXTREAL_SUM_IMAGE f {e} = f e``,
SRW_TAC [][EXTREAL_SUM_IMAGE_INSERT,EXTREAL_SUM_IMAGE_EMPTY,add_rzero]);

val EXTREAL_SUM_IMAGE_POS = store_thm
  ("EXTREAL_SUM_IMAGE_POS",
   ``!f s. FINITE s /\ (!x. x IN s ==> 0 <= f x) ==> 0 <= EXTREAL_SUM_IMAGE f s``,
  Suff `!s. FINITE s ==> (\s. !f. (!x. x IN s ==> 0 <= f x) ==> 0 <= EXTREAL_SUM_IMAGE f s) s`
  >> RW_TAC std_ss []
  ++ MATCH_MP_TAC FINITE_INDUCT
  ++ RW_TAC real_ss [EXTREAL_SUM_IMAGE_EMPTY,le_refl]
  ++ `!x. x IN e INSERT s ==> f x <> NegInf` by METIS_TAC [lt_infty,extreal_of_num_def,extreal_not_infty,lte_trans]
...
val EXTREAL_SUM_IMAGE_SPOS = store_thm
  ("EXTREAL_SUM_IMAGE_SPOS",
   ``!f s. FINITE s /\ (~(s = {})) /\ (!x. x IN s ==> 0 < f x) ==> 0 < EXTREAL_SUM_IMAGE f s``,
  Suff `!s. FINITE s ==> (\s. !f. (~(s = {}))/\ (!x. x IN s ==> 0 < f x) ==> 0 < EXTREAL_SUM_IMAGE f s) s`
  >> RW_TAC std_ss []
  ++ MATCH_MP_TAC FINITE_INDUCT
  ++ RW_TAC std_ss []
  ++ `!x. x IN e INSERT s ==> f x <> NegInf` by METIS_TAC [IN_INSERT,lt_infty,lt_trans,lt_imp_le,extreal_of_num_def]
  ++ RW_TAC std_ss [EXTREAL_SUM_IMAGE_PROPERTY,delete_non_element]
  ++ Cases_on `s = {}`
...
val EXTREAL_SUM_IMAGE_IF_ELIM = store_thm
  ("EXTREAL_SUM_IMAGE_IF_ELIM",
   ``!s P f. FINITE s /\ (!x. x IN s ==> P x) /\ ((!x. x IN s ==> f x <> NegInf) \/ !x. x IN s ==> f x <> PosInf) ==>
		(EXTREAL_SUM_IMAGE (\x. if P x then f x else 0) s =
	 	 EXTREAL_SUM_IMAGE f s)``,
  Suff `!s. FINITE s ==>
	(\s. !P f. (!x. x IN s ==> P x) /\ ((!x. x IN s ==> f x <> NegInf) \/ !x. x IN s ==> f x <> PosInf) ==>
		(EXTREAL_SUM_IMAGE (\x. if P x then f x else 0) s =
 		 EXTREAL_SUM_IMAGE f s)) s`
  >> METIS_TAC []
  ++ MATCH_MP_TAC FINITE_INDUCT
  ++ RW_TAC std_ss [EXTREAL_SUM_IMAGE_EMPTY]
  >> (`!x. x IN e INSERT s ==> (\x. if P x then f x else 0) x <> NegInf` by METIS_TAC [extreal_of_num_def,lt_infty,lt_imp_ne]
     ++ RW_TAC std_ss [EXTREAL_SUM_IMAGE_PROPERTY]
     ++ METIS_TAC [IN_INSERT, DELETE_NON_ELEMENT,lt_infty])
  ++ `!x. x IN (e INSERT s) ==> ((\x. if P x then f x else 0) x <> PosInf)` by METIS_TAC[extreal_of_num_def,lt_infty,lt_imp_ne]
  ++ RW_TAC std_ss [EXTREAL_SUM_IMAGE_PROPERTY]
...
val EXTREAL_SUM_IMAGE_FINITE_SAME = store_thm
  ("EXTREAL_SUM_IMAGE_FINITE_SAME",
   ``!s. FINITE s ==>
	 !f p. p IN s /\ (!q. q IN s ==> (f p = f q)) /\ ((!x. x IN s ==> f x <> NegInf) \/ !x. x IN s ==> f x <> PosInf) ==> (EXTREAL_SUM_IMAGE f s = (&(CARD s)) * f p)``,
  Suff `!s. FINITE s ==>
	 (\s. !f p. p IN s /\ (!q. q IN s ==> (f p = f q)) /\ ((!x. x IN s ==> f x <> NegInf) \/ !x. x IN s ==> f x <> PosInf) ==> (EXTREAL_SUM_IMAGE f s = (&(CARD s)) * f p)) s`
  >> METIS_TAC []
  ++  MATCH_MP_TAC FINITE_INDUCT
...
val EXTREAL_SUM_IMAGE_FINITE_CONST = store_thm
  ("EXTREAL_SUM_IMAGE_FINITE_CONST",
   ``!P. FINITE P ==>
	!f x. (!y. f y = x)  /\ (x <> NegInf \/ x <> PosInf) ==> (EXTREAL_SUM_IMAGE f P = (&(CARD P)) * x)``,
   REPEAT STRIP_TAC
   ...
   ++ RW_TAC std_ss [] >> (ONCE_REWRITE_TAC [EXTREAL_SUM_IMAGE_THM] THEN RW_TAC real_ss [CARD_EMPTY,mul_lzero])
   ++ POP_ASSUM (K ALL_TAC)
...
val EXTREAL_SUM_IMAGE_0 = store_thm
  ...
  ++ `(y IN u)` by PROVE_TAC [CHOICE_DEF]
  ++ `CARD (u DELETE y) = v` by METIS_TAC [CARD_DELETE,FINITE_INSERT,SUC_SUB1]
  ++ METIS_TAC [add_not_infty,FINITE_INSERT,FINITE_DELETE,IN_DELETE,add_rzero]);

val EXTREAL_SUM_IMAGE_IN_IF = store_thm
  ("EXTREAL_SUM_IMAGE_IN_IF",
   ``!s. FINITE s ==>
	!f. ((!x. x IN s ==> f x <> NegInf) \/ !x. x IN s ==> f x <> PosInf) ==> (EXTREAL_SUM_IMAGE f s = EXTREAL_SUM_IMAGE (\x. if x IN s then f x else 0) s)``,
  Suff `!s. FINITE s ==>
		(\s. !f. ((!x. x IN s ==> f x <> NegInf) \/ (!x. x IN s ==> f x <> PosInf)) ==> (EXTREAL_SUM_IMAGE f s = EXTREAL_SUM_IMAGE (\x. if x IN s then f x else 0) s)) s`
  >> RW_TAC std_ss []
  ++ MATCH_MP_TAC FINITE_INDUCT
  ++ RW_TAC real_ss [EXTREAL_SUM_IMAGE_EMPTY]
  >> (`!x. (\x. if x IN e INSERT s then f x else 0) x <> NegInf` by RW_TAC std_ss [extreal_not_infty,extreal_of_num_def]
      ++ FULL_SIMP_TAC real_ss [EXTREAL_SUM_IMAGE_PROPERTY]
      ++ `s DELETE e = s` by rw[GSYM DELETE_NON_ELEMENT]
      ++ `EXTREAL_SUM_IMAGE f s = EXTREAL_SUM_IMAGE (\x. if x IN s then f x else 0) s` by METIS_TAC[IN_INSERT]
      ++ WEAKEN_TAC (equal (Term `!x:'a. x IN e INSERT s ==> f x <> NegInf`))
      (*++ Q.PAT_ASSUM `!x. x IN e INSERT s ==> f x <> NegInf` (K ALL_TAC) *)
      ++ FULL_SIMP_TAC real_ss [IN_INSERT])
  ++ `!x. (\x. if x IN e INSERT s then f x else 0) x <> PosInf` by RW_TAC std_ss [extreal_not_infty,extreal_of_num_def]
  ++ FULL_SIMP_TAC real_ss [EXTREAL_SUM_IMAGE_PROPERTY]
  ++ `s DELETE e = s` by rw [GSYM DELETE_NON_ELEMENT]
  ++ `EXTREAL_SUM_IMAGE f s = EXTREAL_SUM_IMAGE (\x. if x IN s then f x else 0) s` by METIS_TAC [IN_INSERT]
  ++ WEAKEN_TAC (equal (Term `!x:'a. x IN e INSERT s ==> f x <> PosInf`))
  (*++ Q.PAT_ASSUM `!x. x IN e INSERT s ==> f x <> PosInf` (K ALL_TAC)*)
  ++ FULL_SIMP_TAC std_ss [IN_INSERT]);

val EXTREAL_SUM_IMAGE_CMUL = store_thm
  ("EXTREAL_SUM_IMAGE_CMUL",
   ``!s. FINITE s ==>
	!f c. (!x. x IN s ==> f x <> NegInf) \/ (!x. x IN s ==> f x <> PosInf) ==> (EXTREAL_SUM_IMAGE (\x. Normal (c) * f x) s = Normal (c) * (EXTREAL_SUM_IMAGE f s))``,
  Suff `!f c s. FINITE s ==>
	(\s. (!x. x IN s ==> f x <> NegInf) \/ (!x. x IN s ==> f x <> PosInf) ==> (EXTREAL_SUM_IMAGE (\x. Normal (c) * f x) s = Normal (c) * (EXTREAL_SUM_IMAGE f s))) s`
  >> METIS_TAC []
  ...
  >> (Cases_on `0 <= c`
      ...
      ++ `!x. x IN e INSERT s ==> (\x. Normal c * f x) x <> PosInf` by METIS_TAC [mul_not_infty,real_lt,REAL_LT_IMP_LE]
      ++ FULL_SIMP_TAC real_ss [EXTREAL_SUM_IMAGE_PROPERTY,DELETE_NON_ELEMENT]
      ++ METIS_TAC [add_ldistrib_normal,EXTREAL_SUM_IMAGE_NOT_INFTY,IN_INSERT])
  ++ Cases_on `0 <= c`
  >> (`!x. x IN e INSERT s ==> (\x. Normal c * f x) x <> PosInf` by METIS_TAC [mul_not_infty]
      ++ FULL_SIMP_TAC real_ss [EXTREAL_SUM_IMAGE_PROPERTY,DELETE_NON_ELEMENT]
      ++ METIS_TAC [add_ldistrib_normal,EXTREAL_SUM_IMAGE_NOT_INFTY,IN_INSERT])
  ++ `!x. x IN e INSERT s ==> (\x. Normal c * f x) x <> NegInf` by METIS_TAC [mul_not_infty,real_lt,REAL_LT_IMP_LE]
  ++ FULL_SIMP_TAC real_ss [EXTREAL_SUM_IMAGE_PROPERTY,DELETE_NON_ELEMENT]
  ++ METIS_TAC [add_ldistrib_normal,EXTREAL_SUM_IMAGE_NOT_INFTY,IN_INSERT]);
...
val EXTREAL_SUM_IMAGE_IMAGE = store_thm
  ("EXTREAL_SUM_IMAGE_IMAGE",
   ``!s. FINITE s ==>
	!f'. INJ f' s (IMAGE f' s) ==> (!f. (!x. x IN (IMAGE f' s) ==> f x <> NegInf) \/ (!x. x IN (IMAGE f' s) ==> f x <> PosInf) ==> (EXTREAL_SUM_IMAGE f (IMAGE f' s) = EXTREAL_SUM_IMAGE (f o f') s))``,
  Suff `!s. FINITE s ==>
	(\s.!f'. INJ f' s (IMAGE f' s) ==> (!f. (!x. x IN  (IMAGE f' s) ==> f x <> NegInf) \/ (!x. x IN (IMAGE f' s) ==> f x <> PosInf) ==> (EXTREAL_SUM_IMAGE f (IMAGE f' s) = EXTREAL_SUM_IMAGE (f o f') s))) s`
  >> METIS_TAC []
  ++ MATCH_MP_TAC FINITE_INDUCT
  ...
  >> (`FINITE (IMAGE f' s)` by METIS_TAC [IMAGE_FINITE]
      ...
      ++ RW_TAC std_ss [EXTREAL_SUM_IMAGE_PROPERTY]
      ++ `~ (f' e IN IMAGE f' s)`
	by (RW_TAC std_ss [IN_IMAGE] ++ REVERSE (Cases_on `x IN s`) >> ASM_REWRITE_TAC [] ++ METIS_TAC [IN_INSERT])
...
val EXTREAL_SUM_IMAGE_DISJOINT_UNION = store_thm
  ("EXTREAL_SUM_IMAGE_DISJOINT_UNION",
   ``!s s'. FINITE s /\ FINITE s' /\ DISJOINT s s' ==>
		(!f. (!x. x IN s UNION s' ==> f x <> NegInf) \/ (!x. x IN s UNION s' ==> f x <> PosInf) ==> 
  ...
  >>  (FULL_SIMP_TAC std_ss [DISJOINT_SYM]
       ...
       ++ `~ (e' IN (e INSERT s UNION s'))`
  	    by (RW_TAC std_ss[IN_INSERT, IN_UNION] ++ FULL_SIMP_TAC std_ss [EXTREAL_SUM_IMAGE_PROPERTY,DELETE_NON_ELEMENT])
...
val EXTREAL_SUM_IMAGE_EQ_CARD = store_thm
  ...
  ++ RW_TAC std_ss [ADD1,extreal_of_num_def,GSYM REAL_ADD,GSYM extreal_add_def]
  ++ RW_TAC std_ss [GSYM extreal_of_num_def]
...
val EXTREAL_SUM_IMAGE_INV_CARD_EQ_1 = store_thm
  ...
  REPEAT STRIP_TAC
  ++ `(\x. if x IN s then inv (& (CARD s)) else 0) = (\x. inv (& (CARD s)) * (\x. if x IN s then 1 else 0) x)`
	by (RW_TAC std_ss [FUN_EQ_THM] ++ RW_TAC real_ss [mul_rzero,mul_rone])
  ++ POP_ORW
  ++ `inv (&CARD s) = Normal (inv (&CARD s))` 
        by (FULL_SIMP_TAC real_ss [extreal_inv_def,extreal_of_num_def] ++ METIS_TAC [CARD_EQ_0,FINITE_EMPTY])
  ++ POP_ORW
  ++ `!x. (\x. if x IN s then 1 else 0) x <> NegInf` by METIS_TAC [extreal_not_infty,extreal_of_num_def]
  ++ FULL_SIMP_TAC std_ss [EXTREAL_SUM_IMAGE_CMUL,EXTREAL_SUM_IMAGE_EQ_CARD]
  ++ RW_TAC std_ss [extreal_of_num_def,extreal_mul_def]
  ++ `&CARD s <> 0r` by (FULL_SIMP_TAC real_ss [extreal_inv_def,extreal_of_num_def] ++ METIS_TAC [CARD_EQ_0,FINITE_EMPTY])
  ...
  ("EXTREAL_SUM_IMAGE_INTER_NONZERO",
   ``!s. FINITE s ==>
	!f. (!x. x IN s ==> f x <> NegInf) \/ (!x. x IN s ==> f x <> PosInf) ==> 
		 ...
		 EXTREAL_SUM_IMAGE f s)``,
  Suff `!s. FINITE s ==> (\s. !f. (!x. x IN s ==> f x <> NegInf) \/ (!x. x IN s ==> f x <> PosInf) ==>
                (EXTREAL_SUM_IMAGE f (s INTER (\p. ~(f p = 0))) =
		 EXTREAL_SUM_IMAGE f s)) s`
  >> METIS_TAC []
  ++ MATCH_MP_TAC FINITE_INDUCT
...
val EXTREAL_SUM_IMAGE_INTER_ELIM = store_thm
  ("EXTREAL_SUM_IMAGE_INTER_ELIM",
   ``!s. FINITE s ==>
	 !f s'. ((!x. x IN s ==> f x <> NegInf) \/ (!x. x IN s ==> f x <> PosInf)) /\ (!x. (~(x IN s')) ==> (f x = 0)) ==>
			(EXTREAL_SUM_IMAGE f (s INTER s') =
			 EXTREAL_SUM_IMAGE f s)``,
  Suff `!s. FINITE s ==>
	(\s. !f s'. ((!x. x IN s ==> f x <> NegInf) \/ (!x. x IN s ==> f x <> PosInf)) /\ (!x. (~(x IN s')) ==> (f x = 0)) ==>
			(EXTREAL_SUM_IMAGE f (s INTER s') =
			 EXTREAL_SUM_IMAGE f s)) s`
  >> RW_TAC std_ss []
  ++ MATCH_MP_TAC FINITE_INDUCT
...
val EXTREAL_SUM_IMAGE_MONO = store_thm
  ("EXTREAL_SUM_IMAGE_MONO",
   ``!s. FINITE s ==>
	   !f f'. ((!x. x IN s ==> f x <> NegInf /\ f' x <> NegInf) \/ (!x. x IN s ==> f x <> PosInf /\ f' x <> PosInf)) /\ (!x. x IN s ==> f x <= f' x) ==>
		EXTREAL_SUM_IMAGE f s <= EXTREAL_SUM_IMAGE f' s``,
   Suff `!s. FINITE s ==>
		(\s. !f f'. ((!x. x IN s ==> f x <> NegInf /\ f' x <> NegInf) \/ (!x. x IN s ==> f x <> PosInf /\ f' x <> PosInf)) /\  (!x. x IN s ==> f x <= f' x) ==>
...
val EXTREAL_SUM_IMAGE_MONO_SET = store_thm
  ...
  ++ `DISJOINT s (t DIFF s)` by (`DISJOINT s (t DIFF s)` by (rw [DISJOINT_DEF,IN_DIFF,EXTENSION,GSPECIFICATION,NOT_IN_EMPTY,IN_INTER] ++ metis_tac[]) ++ metis_tac[])
  ++ `!x. x IN (s UNION (t DIFF s)) ==> f x <> NegInf` by METIS_TAC [extreal_of_num_def,extreal_not_infty,lt_infty,lte_trans]
  ++ `EXTREAL_SUM_IMAGE f t = EXTREAL_SUM_IMAGE f s + EXTREAL_SUM_IMAGE f (t DIFF s)` by METIS_TAC [EXTREAL_SUM_IMAGE_DISJOINT_UNION]
  ++ POP_ORW
...
val EXTREAL_SUM_IMAGE_POS_MEM_LE = store_thm
  ("EXTREAL_SUM_IMAGE_POS_MEM_LE",
   ``!s. FINITE s ==>
	!f. ((!x. x IN s ==> f x <> NegInf) \/ (!x. x IN s ==> f x <> PosInf)) /\ (!x. x IN s ==> 0 <= f x) ==>
	    (!x. x IN s ==> f x <= EXTREAL_SUM_IMAGE f s)``,
  Suff `!s. FINITE s ==>
	(\s. !f. ((!x. x IN s ==> f x <> NegInf) \/ (!x. x IN s ==> f x <> PosInf)) /\ (!x. x IN s ==> 0 <= f x) ==>
	    (!x. x IN s ==> f x <= EXTREAL_SUM_IMAGE f s)) s`
  >> RW_TAC std_ss []
  ++ MATCH_MP_TAC FINITE_INDUCT
  ++ RW_TAC std_ss [NOT_IN_EMPTY]
  >> (FULL_SIMP_TAC std_ss [EXTREAL_SUM_IMAGE_PROPERTY,DELETE_NON_ELEMENT]
      ++ `!x. x IN s ==> f x <> NegInf` by METIS_TAC [IN_INSERT]
      ++ `(!x. x IN s ==> 0 <= f x)` by METIS_TAC [IN_INSERT]
      ++ FULL_SIMP_TAC std_ss [IN_INSERT]
  ...
  ++ FULL_SIMP_TAC std_ss [EXTREAL_SUM_IMAGE_PROPERTY,DELETE_NON_ELEMENT]
  ++ `!x. x IN s ==> f x <> PosInf` by METIS_TAC [IN_INSERT]
  ++ `(!x. x IN s ==> 0 <= f x)` by METIS_TAC [IN_INSERT]
  ++ FULL_SIMP_TAC std_ss [IN_INSERT]
  >> METIS_TAC [EXTREAL_SUM_IMAGE_POS,le_add2,add_rzero,extreal_of_num_def,extreal_not_infty,le_refl]
  ++ `f x <= EXTREAL_SUM_IMAGE f s` by FULL_SIMP_TAC std_ss [IN_INSERT]
...
val EXTREAL_SUM_IMAGE_ADD = store_thm
  ("EXTREAL_SUM_IMAGE_ADD",
   ``!s. FINITE s ==> !f f'. ((!x. x IN s ==> f x <> NegInf /\ f' x <> NegInf) \/
                   (!x. x IN s ==> f x <> PosInf /\ f' x <> PosInf)) ==> 
...
val EXTREAL_SUM_IMAGE_SUB = store_thm
  ("EXTREAL_SUM_IMAGE_SUB",
   ``!s. FINITE s ==> !f f'. ((!x. x IN s ==> f x <> NegInf /\ f' x <> PosInf) \/
  ...
  >> (`FINITE (e INSERT s)` by FULL_SIMP_TAC std_ss [FINITE_INSERT]
      ...
      ++ POP_ORW
      ++ `Normal (-1) * EXTREAL_SUM_IMAGE f' (e INSERT s) = EXTREAL_SUM_IMAGE (\x. Normal (-1) * f' x) (e INSERT s)`
            by METIS_TAC [EXTREAL_SUM_IMAGE_CMUL]
...
val EXTREAL_SUM_IMAGE_EXTREAL_SUM_IMAGE = store_thm
  ("EXTREAL_SUM_IMAGE_EXTREAL_SUM_IMAGE",
   ``!s s' f. FINITE s /\ FINITE s' /\ (!x. x IN s CROSS s' ==> f (FST x) (SND x) <> NegInf) ==>
	(EXTREAL_SUM_IMAGE (\x. EXTREAL_SUM_IMAGE (f x) s') s =
	 EXTREAL_SUM_IMAGE (\x. f (FST x) (SND x)) (s CROSS s'))``,
   Suff `!s. FINITE s ==>
	     (\s. !s' f. FINITE s' /\ (!x. x IN s CROSS s' ==> f (FST x) (SND x) <> NegInf) ==>
	(EXTREAL_SUM_IMAGE (\x. EXTREAL_SUM_IMAGE (f x) s') s =
	 EXTREAL_SUM_IMAGE (\x. f (FST x) (SND x)) (s CROSS s'))) s`
   >> RW_TAC std_ss []
   ++ MATCH_MP_TAC FINITE_INDUCT
   ++ RW_TAC std_ss [CROSS_EMPTY, EXTREAL_SUM_IMAGE_EMPTY, DELETE_NON_ELEMENT]
   ...
   ++ (MP_TAC o Q.SPEC `(\x. f (FST x) (SND x))` o UNDISCH o UNDISCH o UNDISCH o REWRITE_RULE [GSYM AND_IMP_INTRO] o Q.ISPECL [`IMAGE (\x. (e:'a,x)) (s':'b->bool)`, `(s:'a->bool) CROSS (s':'b->bool)`]) EXTREAL_SUM_IMAGE_DISJOINT_UNION

   ++ `(!x. x IN IMAGE (\x. (e,x)) s' UNION s CROSS s' ==>
           f (FST x) (SND x) <> NegInf)`
          by (FULL_SIMP_TAC std_ss [IN_IMAGE,IN_UNION,IN_INSERT,IN_CROSS]
              ++ METIS_TAC [FST,SND])
   ++ RW_TAC std_ss []
   ++ `INJ (\x. (e,x)) s' (IMAGE (\x. (e,x)) s')` by RW_TAC std_ss [INJ_DEF, IN_IMAGE]
   ++ (MP_TAC o Q.SPEC `(\x. f (FST x) (SND x))` o UNDISCH o Q.SPEC `(\x. (e,x))` o UNDISCH o Q.SPEC `s'` o 
   ...
   ++ (MP_TAC o Q.SPEC `e` o UNDISCH o Q.SPECL [`(\x. EXTREAL_SUM_IMAGE (f x) s')`,`s`]) EXTREAL_SUM_IMAGE_PROPERTY
   ++ RW_TAC std_ss []
...
val EXTREAL_SUM_IMAGE_NORMAL = store_thm
  ...
  >> RW_TAC std_ss []
  ++ MATCH_MP_TAC FINITE_INDUCT
  ++ RW_TAC std_ss [EXTREAL_SUM_IMAGE_EMPTY,REAL_SUM_IMAGE_THM,extreal_of_num_def,REAL_SUM_IMAGE_THM,GSYM extreal_add_def]
...
("EXTREAL_SUM_IMAGE_IN_IF_ALT",``!s f z. FINITE s /\ ((!x. x IN s ==> f x <> NegInf) \/ (!x. x IN s ==> f x <> PosInf)) ==>
     (EXTREAL_SUM_IMAGE f s = EXTREAL_SUM_IMAGE (\x. if x IN s then f x else z) s)``,
  Suff `!s. FINITE s ==> (\s. !f z. ((!x. x IN s ==> f x <> NegInf) \/ (!x. x IN s ==> f x <> PosInf)) ==>
     (EXTREAL_SUM_IMAGE f s = EXTREAL_SUM_IMAGE (\x. if x IN s then f x else z) s)) s`
  >> METIS_TAC []
  ++ MATCH_MP_TAC FINITE_INDUCT
  ++ RW_TAC std_ss [EXTREAL_SUM_IMAGE_EMPTY]
  >> (`!i. i IN e INSERT s ==> (\x. if x IN e INSERT s then f x else z) i <> NegInf` by RW_TAC std_ss []
      ...
      >> RW_TAC std_ss []
      ++ `EXTREAL_SUM_IMAGE f s = EXTREAL_SUM_IMAGE (\x. if x IN s then f x else z) s` by METIS_TAC [IN_INSERT]
      ++ POP_ORW
...
val extreal_inf_def = Define
  `extreal_inf p =
   if !x. (!y. p y ==> x <= y) ==> (x = NegInf) then NegInf
   else (if !x. p x ==> (x = PosInf) then PosInf 
             else Normal (inf (\r. p (Normal r))))`;
...
val sup_le = store_thm
  ...
  << [METIS_TAC [le_infty,extreal_not_infty],
	  ...
	  >> (RW_TAC std_ss []
		  ...
		  RW_TAC std_ss [extreal_le_def]
                  ++ MATCH_MP_TAC REAL_LE_TRANS
		  ++ Q.EXISTS_TAC `real$sup (\r. p (Normal r))`
		  ++ RW_TAC std_ss []
		  ++ MATCH_MP_TAC REAL_IMP_LE_SUP
		  ++ CONJ_TAC >> METIS_TAC []
		  ++ REVERSE CONJ_TAC >> (Q.EXISTS_TAC `r''` ++ RW_TAC real_ss [])
	   ...
	   ++ RW_TAC std_ss []
	   >> (Cases_on `x''` << [RW_TAC std_ss [], METIS_TAC [le_infty,extreal_not_infty], METIS_TAC []])
           ++ METIS_TAC [extreal_le_def]]]);
...
val sup_eq = store_thm
  ("sup_eq", ``!p x. (sup p = x) = (!y. p y ==> y <= x) /\ !y. (!z. p z ==> z <= y) ==> x <= y``,
...
val sup_seq = store_thm
 ("sup_seq", ``!f l. mono_increasing f ==> ((f --> l)  = (sup (IMAGE (\n. Normal (f n)) UNIV) = Normal l))``,
  RW_TAC std_ss []
  ++ EQ_TAC
  >> (RW_TAC std_ss [sup_eq]
      >> (POP_ASSUM (MP_TAC o ONCE_REWRITE_RULE [GSYM SPECIFICATION])
  ...
  ++ RW_TAC std_ss []
  ++ `?x. x IN IMAGE f univ(:num) /\ real$sup (IMAGE f univ(:num)) <= x + e / 2` by METIS_TAC []
  ...
  >> (MATCH_MP_TAC REAL_LET_TRANS ++ Q.EXISTS_TAC `f x'''''' + e / 2`
   ...
   ++ MATCH_MP_TAC REAL_LET_TRANS ++ Q.EXISTS_TAC `real$sup (IMAGE f UNIV)`
   ++ RW_TAC std_ss [REAL_LT_ADDR]
   ++ Suff `!y. (\y. y IN IMAGE f UNIV) y ==> y <= real$sup (IMAGE f UNIV)`
   >> METIS_TAC [IN_IMAGE, IN_UNIV]
   ++ SIMP_TAC std_ss [IN_DEF]
   ++ MATCH_MP_TAC REAL_SUP_UBOUND_LE
   ++ `!y x z. IMAGE f UNIV x = x IN IMAGE f UNIV` by RW_TAC std_ss [IN_DEF]
...
val sup_add_mono = store_thm
 ("sup_add_mono", ``!f g. (!n. 0 <= f n) /\ (!n. f n <= f (SUC n)) /\ (!n. 0 <= g n) /\ (!n. g n <= g (SUC n)) ==>
  ...
  ++ `!n. g n <> NegInf` by METIS_TAC [le_trans,le_infty,lt_infty,lte_trans,extreal_of_num_def,extreal_not_infty]
  ++ `!n. f n <> PosInf` by METIS_TAC [le_trans,lt_infty,let_trans]
  ++ `!n. f n <> NegInf` by METIS_TAC [le_trans,le_infty,lt_infty,lte_trans,extreal_of_num_def,extreal_not_infty]
  ++ MATCH_MP_TAC le_trans
  ...
  >> (POP_ASSUM (MP_TAC o ONCE_REWRITE_RULE [GSYM SPECIFICATION]) 
      ...
      ++ CONJ_TAC >> METIS_TAC [le_ladd,ext_mono_increasing_def,ext_mono_increasing_suc,le_refl,NOT_LEQ,le_trans]
      ++ Q.PAT_ASSUM `!z. Q z ==> z <= y` MATCH_MP_TAC
      ++ ONCE_REWRITE_TAC [GSYM SPECIFICATION]
      ++ RW_TAC std_ss [IN_IMAGE,IN_UNIV]
      ++ METIS_TAC [])
...
val sup_sum_mono = store_thm
 ("sup_sum_mono", ``!f s. FINITE s /\ (!i:num. i IN s ==> (!n. 0 <= f i n)) /\
                      ...
                      (sup (IMAGE (\n. SIGMA (\i:num. f i n) s) UNIV) = SIGMA (\i:num. sup (IMAGE (f i) UNIV)) s)``,
  Suff `!s. FINITE s ==> (\s. !f. (!i:num. i IN s ==> (!n. 0 <= f i n)) /\
                        (!i:num. i IN s ==> (!n. f i n <= f i (SUC n))) ==>  
                      (sup (IMAGE (\n. SIGMA (\i:num. f i n) s) UNIV) = SIGMA (\i:num. sup (IMAGE (f i) UNIV)) s)) s`
  >> RW_TAC std_ss []
  ++ MATCH_MP_TAC FINITE_INDUCT
  ...
  ++ `!n. SIGMA (\i. f i n) (e INSERT s) = (\i. f i n) e + SIGMA (\i. f i n) (s DELETE e)`
        by (STRIP_TAC
            ++ (MATCH_MP_TAC o Q.SPEC `e` o UNDISCH o Q.SPECL [`(\i. f i n)`,`s`] o INST_TYPE [alpha |-> ``:num``]) EXTREAL_SUM_IMAGE_PROPERTY
	    ++ METIS_TAC [IN_INSERT,le_infty,lt_infty,extreal_of_num_def,extreal_not_infty])
  ++ RW_TAC std_ss []
  ++ `!n. !x. x IN e INSERT s ==> f x n <> NegInf` by METIS_TAC [IN_INSERT,le_infty,lt_infty,extreal_of_num_def,extreal_not_infty]
  ++ `sup (IMAGE (\n. f e n + (\n. SIGMA (\i. f i n) s) n) UNIV) =
      sup (IMAGE (f e) UNIV) + sup (IMAGE (\n. SIGMA (\i. f i n) s) UNIV)`
        by ((MATCH_MP_TAC o Q.SPECL [`f e`, `(\n. SIGMA (\i. f i n) s)`] o INST_TYPE [alpha |-> ``:num``]) sup_add_mono 
            ++ FULL_SIMP_TAC std_ss [IN_INSERT,EXTREAL_SUM_IMAGE_POS]
	    ++ RW_TAC std_ss []
	    ++ (MATCH_MP_TAC o UNDISCH o Q.SPEC `s` o INST_TYPE [alpha |-> ``:num``]) EXTREAL_SUM_IMAGE_MONO
	    ++ FULL_SIMP_TAC std_ss [IN_INSERT])
  ++ FULL_SIMP_TAC std_ss []
...
val sup_le_mono = store_thm
 ("sup_le_mono",``!f z. (!n. f n <= f (SUC n)) /\ z < sup (IMAGE f UNIV) ==> ?n. z <= f n``,
  RW_TAC std_ss []
  ++ SPOSE_NOT_THEN ASSUME_TAC
...
val sup_cmul = store_thm
 ("sup_cmul",``!f c. 0 <= c ==> (sup (IMAGE (\n. (Normal c) * f n) UNIV) = (Normal c) * sup (IMAGE f UNIV))``,
  RW_TAC std_ss []
  ++ Cases_on `c = 0` >> RW_TAC real_ss [mul_lzero, GSYM extreal_of_num_def,UNIV_NOT_EMPTY,sup_const_over_set]
...
("sup_lt_epsilon",``!P e. (0 < e) /\ (?x. P x /\ x <> NegInf) /\ (sup P <> PosInf) ==> (?x. P x /\ sup P < x + e)``,
  RW_TAC std_ss []
  ++ Cases_on `e = PosInf`
  >> (Q.EXISTS_TAC `x`
      ++ RW_TAC std_ss []
      ++ METIS_TAC [extreal_add_def,lt_infty,extreal_cases])
  ++ `e <> NegInf` by METIS_TAC [le_sup_imp,lt_infty,lte_trans,extreal_of_num_def,extreal_not_infty]
  ++ `sup P <> NegInf` by METIS_TAC [le_sup_imp,lt_infty,lte_trans]
  ++ `sup P < sup P + e` 
     by (Cases_on `sup P` ++ Cases_on `e` ++ RW_TAC std_ss [extreal_cases,extreal_add_def,extreal_lt_def,extreal_le_def,GSYM real_lt] 
  ...
  ++ FULL_SIMP_TAC std_ss [extreal_add_def,extreal_lt_def,extreal_le_def,GSYM real_lt,REAL_LT_SUB_RADD]);
...
val inf_seq = store_thm
 ("inf_seq", ``!f l. mono_decreasing f ==> ((f --> l)  = (inf (IMAGE (\n. Normal (f n)) UNIV) = Normal l))``,
  RW_TAC std_ss []
  ++ EQ_TAC
  >> (RW_TAC std_ss [inf_eq]
      >> (POP_ASSUM (MP_TAC o ONCE_REWRITE_RULE [GSYM SPECIFICATION])
...
val inf_cminus = store_thm
 ("inf_cminus", ``!f c. Normal c - inf (IMAGE f UNIV) = sup (IMAGE (\n. Normal c - f n) UNIV)``,
  RW_TAC std_ss [sup_eq]
  >> (POP_ASSUM (MP_TAC o ONCE_REWRITE_RULE [GSYM SPECIFICATION])
      ++ RW_TAC std_ss [IN_IMAGE,IN_UNIV]
...
("ext_suminf_add",``!f g. (!n. 0 <= f n /\ 0 <= g n) ==> (suminf (\n. f n + g n) = suminf f + suminf g)``,
  ...
  ++ `!m. sup (IMAGE (\n. SIGMA g (count n)) univ(:num)) <= y - sup (IMAGE (\n. SIGMA f (count n)) univ(:num))`
       by (RW_TAC std_ss [sup_le]
           ++ POP_ASSUM (MP_TAC o ONCE_REWRITE_RULE [GSYM SPECIFICATION])
	   ++ RW_TAC std_ss [IN_IMAGE,IN_UNIV]
	   ++ FULL_SIMP_TAC std_ss [])
...
val ext_suminf_cmul = store_thm
 ("ext_suminf_cmul",``!f c. 0 <= c /\ (!n. 0 <= f n) ==> (ext_suminf (\n. c * f n) = c * ext_suminf f)``,
  ...
  >> (RW_TAC std_ss [extreal_mul_def,extreal_of_num_def,EXTREAL_SUM_IMAGE_0,FINITE_COUNT]
      ++ `sup (IMAGE (\n. Normal 0) univ(:num)) = 0` 
           by (MATCH_MP_TAC sup_const_alt
               ++ RW_TAC std_ss []
	       >> (Q.EXISTS_TAC `Normal 0` ++ ONCE_REWRITE_TAC [GSYM SPECIFICATION] ++ RW_TAC std_ss [IN_IMAGE,IN_UNIV])
	       ++ POP_ASSUM (MP_TAC o ONCE_REWRITE_RULE [GSYM SPECIFICATION])
  ...
  ++ `PosInf * f n <= SIGMA (\n. PosInf * f n) (count (SUC n))`
	  ...
	  ++ (MP_TAC o REWRITE_RULE [FINITE_COUNT] o Q.ISPECL [`(\n:num. PosInf * f n)`, `count (SUC n)`]) EXTREAL_SUM_IMAGE_POS_MEM_LE
	  ++ RW_TAC std_ss [])
  ...
  ++ `SIGMA (\n. PosInf * f n) (count (SUC n)) <= sup (IMAGE (\n. SIGMA (\n. PosInf * f n) (count n)) univ(:num))`
        by (MATCH_MP_TAC le_sup_imp
            ++ ONCE_REWRITE_TAC [GSYM SPECIFICATION] 
            ++ RW_TAC std_ss [IN_IMAGE,IN_UNIV]
	    ++ METIS_TAC [])
  ++ `sup (IMAGE (\n. SIGMA (\n. PosInf * f n) (count n)) univ(:num)) = PosInf` by METIS_TAC [le_infty]
  ++ METIS_TAC []);   

val ext_suminf_cmul_alt = store_thm
 ("ext_suminf_cmul_alt",``!f c. 0 <= c /\ ((!n. f n <> NegInf) \/ (!n. f n <> PosInf)) ==> (ext_suminf (\n. (Normal c) * f n) = (Normal c) * ext_suminf f)``,
  RW_TAC std_ss [ext_suminf_def]
...
val ext_suminf_lt_infty = store_thm
 ("ext_suminf_lt_infty",``!f. (!n. 0 <= f n) /\ ext_suminf f <> PosInf ==> (!n. f n < PosInf)``,
  RW_TAC std_ss [ext_suminf_def]
...
val ext_suminf_mono = store_thm
 ("ext_suminf_mono",``!f g. (!n. g n <> NegInf /\ g n <= f n) ==> (ext_suminf g <= ext_suminf f)``,
  ...
  ++ RW_TAC std_ss []
  >> (MATCH_MP_TAC ((REWRITE_RULE [FINITE_COUNT] o Q.ISPEC `count n`) EXTREAL_SUM_IMAGE_MONO)
      ++ RW_TAC std_ss []
      ++ DISJ1_TAC
...
("ext_suminf_sub",``!f g. (!n. 0 <= g n /\ g n <= f n) /\ ext_suminf f <> PosInf ==> 
      (ext_suminf (\i. f i - g i) = ext_suminf f - ext_suminf g)``,
  RW_TAC std_ss []
  ++ `ext_suminf g <= ext_suminf f` 
      by (RW_TAC std_ss [ext_suminf_def,sup_le,le_sup]
          ++ Q.PAT_ASSUM `IMAGE (\n. SIGMA g (count n)) univ(:num) y` (MP_TAC o ONCE_REWRITE_RULE [GSYM SPECIFICATION])
	  ++ RW_TAC std_ss [IN_IMAGE,IN_UNIV]
          ++ MATCH_MP_TAC le_trans
	  ++ Q.EXISTS_TAC `SIGMA f (count n)`
	  ++ RW_TAC std_ss []
  ...
  ++ FULL_SIMP_TAC std_ss []
  ++ FULL_SIMP_TAC std_ss [extreal_sub_def,extreal_le_def,extreal_not_infty,extreal_of_num_def]
  ...
  ++ `0 <= y` by METIS_TAC [REAL_SUM_IMAGE_POS,FINITE_COUNT,extreal_le_def,extreal_of_num_def,le_trans]
  ++ `0 <= y'` by METIS_TAC [REAL_SUM_IMAGE_POS,FINITE_COUNT,extreal_le_def,extreal_of_num_def,le_trans]
  ++ `0 <= SIGMA (\n. p n - q n) (count n)` by (MATCH_MP_TAC REAL_SUM_IMAGE_POS ++ RW_TAC std_ss [FINITE_COUNT])
  ++ `0 <= y''` by METIS_TAC [extreal_le_def,extreal_of_num_def,le_trans]
  ++ `y <> NegInf /\ y' <> NegInf /\ y'' <> NegInf` by METIS_TAC [lt_infty,num_not_infty,lte_trans]
  ++ `?z. y = Normal z` by METIS_TAC [extreal_cases]
  ++ `?z'. y' = Normal z'` by METIS_TAC [extreal_cases]
  ++ `?z''. y'' = Normal z''` by METIS_TAC [extreal_cases]
...
("ext_suminf_sum",``!f n. (!n. 0 <= f n) /\ (!m. n <= m ==> (f m = 0)) ==> (suminf f = SIGMA f (count n))``,
  RW_TAC std_ss [ext_suminf_def,sup_eq]
  >> (POP_ASSUM (MP_TAC o ONCE_REWRITE_RULE [GSYM SPECIFICATION])
      ++ RW_TAC std_ss [IN_IMAGE,IN_UNIV]
      ++ Cases_on `n' <= n`
      ...
      ++ (MP_TAC o  REWRITE_RULE [FINITE_COUNT] o Q.ISPECL [`count n`,`count n' DIFF count n`]) EXTREAL_SUM_IMAGE_DISJOINT_UNION
      ++ RW_TAC std_ss []
...
val min_infty = store_thm
  ("min_infty",
   ``!x. (min x PosInf = x) /\ (min PosInf x = x) /\ (min NegInf x = NegInf) /\ (min x NegInf = NegInf)``,
...
val max_infty = store_thm
  ("max_infty",
   ``!x. (max x PosInf = PosInf) /\ (max PosInf x = PosInf) /\ (max NegInf x = x) /\ (max x NegInf = x)``,
...
val Q_COUNTABLE = store_thm
  ...
  >> (RW_TAC std_ss [COUNTABLE_ALT]
      ...
      ++ RW_TAC std_ss []
      ++ Q.EXISTS_TAC  `(\ (a:num,b:num). (extreal_of_num a) /(extreal_of_num b)) o f`	
...
val DIV_IN_Q = store_thm
  ...
  ++  METIS_TAC [MUL_IN_Q,extreal_mul_def, extreal_of_num_def]);
...
val COUNTABLE_ENUM_Q = store_thm
  ...
  ++ `IMAGE (\u. if u = x then e else f u) (Q_set DIFF {x}) = IMAGE f Q_set` by (rw [IMAGE_DEF,EXTENSION,GSPECIFICATION,DIFF_DEF,IN_UNION,IN_SING] ++ metis_tac[])
